...
#' @export
smwhc<-function(result,n.rand=99,ws,target=c("marginal_gain","wss"),conf=0.95, session=getDefaultReactiveDomain()){
  target<-match.arg(target)
  result0<-result
  if(is.null(result0$Clusters)){
    result0$Clusters<-result0[,1]
  }
  if(target=="marginal_gain"){
    yo<- -diff(result$WSS)
    yo_clusters<-result0$Clusters[-1]
  } else{
    yo<-result$WSS
    yo_clusters<-result0$Clusters
  }
  smw<-list()
  yo<-data.frame(yo)
  ...
  withProgress(max =length(ws),session=session,{
    for (j in 1:length(ws)) {
      ...
      DPtable$clusters<-yo_clusters[DPtable$positions]
      OB<-DPtable[, 3]
      ...
      SD<-apply(rdp, 1, sd)
      Q<-apply(rdp,1,function(x) quantile(x,probs=conf,na.rm=T,names=F))
      P<-sapply(seq_along(OB),function(i) mean(rdp[i,]>=OB[i],na.rm=T))
      oem<-sum(Dmean)/(nrow(yo) - w1)
      osd<-sum(SD)/(nrow(yo) - w1)
      Dz<-(OB - oem)/osd
      DPtable$zscore<-Dz
      DPtable$sd<-SD
      DPtable$random_mean<-Dmean
      DPtable$random_quantile<-Q
      DPtable$p_value<-P
  ...
  result<-data.frame(Clusters=result0$Clusters)
  sds<-data.frame(Clusters=result0$Clusters)
  qs<-data.frame(Clusters=result0$Clusters)
  ps<-data.frame(Clusters=result0$Clusters)
  for(i in 1:length(smw) ){
    x<-smw[[i]]
    x_match<-match(x$dp$clusters,result0$Clusters)
    result[x_match, names(smw)[i]]<-x$dp$diss
    sds[x_match, names(smw)[i]]<-x$dp$sd
    qs[x_match, names(smw)[i]]<-x$dp$random_quantile
    ps[x_match, names(smw)[i]]<-x$dp$p_value
  }


  sd<-apply(sds[,-1,drop=F],1,function(x) mean(x,na.rm=T))
  dis<-apply(result[,-1,drop=F],1,function(x) mean(x,na.rm=T))
  q<-apply(qs[,-1,drop=F],1,function(x) mean(x,na.rm=T))
  p<-apply(ps[,-1,drop=F],1,function(x) mean(x,na.rm=T))
  distab<-data.frame(dis,sd,q,p)
  colnames(distab)<-c("Mean Diss","SD Diss","Random Quantile","SMW p")

  out<-cbind(result0,distab)
  attr(out,"smw_windows")<-ws
  attr(out,"smw_target")<-target
  attr(out,"smw_conf")<-conf
  if(length(ws)>0){
    max_window<-max(ws,na.rm=T)
    attr(out,"smw_full_window_range")<-yo_clusters[c(max_window/2,nrow(yo)-max_window/2)]
  }
  out
}
#' @export
get_screeplot_smw_sig<-function(smw, tol=NULL, threshold_method=c("random_quantile","tol_sd")){
  ...
  smw2$Sig<-NA
  smw2$smw_valid<-TRUE
  full_window_range<-attr(smw,"smw_full_window_range")
  if(!is.null(full_window_range)&&length(full_window_range)==2&&!any(is.na(full_window_range))){
    edge_rows<-smw2$Clusters<full_window_range[1]|smw2$Clusters>full_window_range[2]
    smw2$smw_valid[edge_rows]<-FALSE
  }
  if(threshold_method=="random_quantile"){
    conf<-attr(smw,"smw_conf")
    if(is.null(conf)||is.na(conf)){conf<-0.95}
    label<-paste0("Mean Diss>Random ",round(100*conf),"%")
    pic<-which(!is.na(smw2[["Mean Diss"]])&!is.na(smw2[["Random Quantile"]])&
                 smw2[["Mean Diss"]]>smw2[["Random Quantile"]])
  } else{
    label<-paste0("Mean Diss>",tol,"*","SD")
    pic<-which(!is.na(smw2[["Mean Diss"]])&!is.na(smw2[["SD Diss"]])&
                 smw2[["Mean Diss"]]>tol*smw2[["SD Diss"]])
  }
  if(length(pic)>0){
    smw2[pic,"Sig"]<-label
  }
  attr(smw2,"smw_sig_label")<-label

  smw2
}
#' @export
get_ggdata_screesms<-function(smw2, tol=NULL, threshold_method=c("random_quantile","tol_sd")){
  threshold_method<-match.arg(threshold_method)
  tol_level<-attr(smw2,"smw_sig_label")
  if(is.null(tol_level)){
    tol_level<-if(threshold_method=="random_quantile"){"Mean Diss>Random threshold"} else{paste0("Mean Diss>",tol,"*","SD")}
  }
  smw3<-smw2
  wss<-data.frame(x=smw3$Clusters,y=smw3$WSS)
  wss$color<-"WSS"
  diss<-data.frame(x=smw3$Clusters,y=smw3[["Mean Diss"]])
  diss$color<-"Dissimilarity"
  sigs<-data.frame(x=smw3$Clusters,y=smw3[["Mean Diss"]], color=smw3$Sig)
  wss$smw_valid<-smw3$smw_valid
  diss$smw_valid<-smw3$smw_valid
  sigs$smw_valid<-smw3$smw_valid
  wss$group<-"wss"
  diss$group<-"diss"
  sigs$group<-"sigs"
...
scree_smw_ggplot<-function(df){
  diss_df<-df[df$color=="Dissimilarity"&!is.na(df$y),,drop=F]
  if("smw_valid"%in%names(diss_df)){
    diss_df<-diss_df[!is.na(diss_df$smw_valid)&diss_df$smw_valid,,drop=F]
  }
  sig_df<-df[df$group=="sigs"&!is.na(df$y)&!is.na(df$color),,drop=F]
  if("smw_valid"%in%names(sig_df)){
    sig_df<-sig_df[!is.na(sig_df$smw_valid)&sig_df$smw_valid,,drop=F]
  }
  maxdiss<-integer(0)
  if(nrow(sig_df)>0){
    maxdiss<-sig_df$x[which.max(sig_df$y)]
  } else if(nrow(diss_df)>0){
    maxdiss<-diss_df$x[which.max(diss_df$y)]
  }
...
plotnetwork_list2<-function(m,label=T, main="", show_points=T){
  ...
  cols<-viridis::viridis(nlevels(cut))[cut]
...
bmu_plot_hc<-function(m,hexs,points_tomap=NULL,bp=NULL,points=T,points_size=2,points_palette="turbo",pch=16,text=F,text_factor=NULL,text_size=1.5,text_palette="turbo",bg_palette="viridis",newcolhabs= vals$newcolhabs,bgalpha=1,border="white",indicate=NULL,cex.var=1,col.text="black",col.bg.var="white",col.bg.var.alpha=.8, newdata=NULL, show_error=NULL,base_size=12,show_neucoords=T,title="", hc=NULL, plotY=F,Y_palette="turbo", property=NULL,fill_neurons=F,border_width=0.5,var_pie=F,var_pie_type="top",var_pie_transp=0.1,n_var_pie=5,var_pie_layer=1,pie_variables=1:2,text_repel=F,max.overlaps=10,show_legend=T,neuron_legend="Group",points_legend="Observations") {
  ...
  if(!is.null(points_tomap)){
    ...
    if(isTRUE(text)){
      req(length(points_tomap)>0)
      coltext<-getcolhabs(newcolhabs,text_palette,1)
...
bmu_plot<-function(m,hexs,points_tomap=NULL,bp=NULL,points=T,points_size=2,points_palette="turbo",pch=16,text=F,text_factor=NULL,text_size=1.5,text_palette="turbo",bg_palette="viridis",newcolhabs= vals$newcolhabs,bgalpha=1,border="white",indicate=NULL,cex.var=1,col.text="black",col.bg.var="white",col.bg.var.alpha=.8, newdata=NULL, show_error=NULL,base_size=12,show_neucoords=T,title="", hc=NULL, plotY=F,Y_palette="turbo", property=NULL,fill_neurons=F,border_width=0.5,
  ...
  if(!is.null(points_tomap)){
    if(isTRUE(points)){
      ...
      p<- p+geom_point(data=points_tomap, aes(x_pt, y_pt,col=point), size=points_size+2, shape=pch)+
        scale_color_manual(name=points_legend,values=colpoints,guide=guide)
    }
