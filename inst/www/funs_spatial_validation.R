...
resampling_label <- function(m, numbers = TRUE, short_cvsp = FALSE) {
  ...
  time_cv <- attr(m, "time_cv")

  if (!is.null(time_cv)) {

    scheme <- time_cv$time_scheme

    scheme_label <- if (identical(scheme, "rolling")) {
      "Rolling Window Temporal Cross-Validation"
    } else if (identical(scheme, "expanding")) {
      "Expanding Window Temporal Cross-Validation"
    } else {
      "Temporal Cross-Validation"
    }

    n_folds <- time_cv$n_folds
    initial_n <- time_cv$initial_n
    horizon_n <- time_cv$horizon_n
    skip_n <- time_cv$skip_n
    advance_n <- time_cv$advance_n
    caret_skip_n <- time_cv$caret_skip_n
    time_var <- time_cv$time_var

    if (is.null(n_folds) || is.na(n_folds)) {
      n_folds <- length(m$control$index)
    }
    ...
    if (numbers) {
      ...
      return(
        paste0(
          ...
          "; initial window = ", initial_n,
          "; test window = ", horizon_n,
          "; skip after test = ", skip_n,
          "; advance = ", advance_n,
          "; caret skip = ", caret_skip_n,
          ")"
        )
      )
