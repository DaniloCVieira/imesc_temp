...
option_tips <- {
  list(
    ...
    "Create temporal features in the Numeric-Attribute for modelling.",
...
create_DATALIST <- function(
  ...
  factors_in <- factors[rownames(data), , drop = FALSE]

  validate(
    need(
      nrow(factors_in) == nrow(data),
      ""
    )
  )

  is_time_col <- function(x) {
    inherits(x, c("Date", "POSIXct", "POSIXlt", "hms", "difftime"))
  }

  time_cols <- which(unlist(lapply(data, is_time_col)))

  if (length(time_cols) > 0) {

    data.time <- data[, time_cols, drop = FALSE]
    rownames(data.time) <- rownames(data)

    if (is.null(time)) {
      time <- data.time
    } else {
      time <- time[rownames(data), , drop = FALSE]

      duplicated_names <- colnames(data.time) %in% colnames(time)

      if (any(duplicated_names)) {
        colnames(data.time)[duplicated_names] <- make.unique(
          c(colnames(time), colnames(data.time)[duplicated_names])
        )[(ncol(time) + 1):(ncol(time) + sum(duplicated_names))]
      }

      time <- cbind(time, data.time)
    }

    data <- data[, -time_cols, drop = FALSE]

    attr(data, "data.time") <- "time columns removed from numeric-attribute"
  }
  ...
  if (any(names(datastr(data)$type.vars) == "factor")) {
    ...
    for (i in 1:ncol(data)) {
      ...
      if (!sum(is.na(new)) == length(new)) {
        data[, i] <- new
      }
    }

    num_cols <- which(unlist(lapply(data, function(x) is.numeric(x))))

    data.numerics <- data[, num_cols, drop = FALSE]

    data.factors <- data[
      ,
      which(unlist(lapply(data, function(x) is.factor(x)))),
      drop = FALSE
    ]
  ...
  validate(
    need(
      ncol(data) > 0,
      "Numeric-Attribute must contain at least one numeric column after removing factor and time columns."
    )
  )
  ...
  if (up_or_ex == "example") {
    ...
    attr(data, "filename") <- data_name
    attr(data, "datalist") <- data_name
  }

  validate(
    need(
      sum(rownames(factors_in) %in% rownames(data)) > 0,
      "Numeric-Attribute and Factors-Attribute must have conforming IDs (first column). Check these files and reload again;"
    )
  )
  ...
  if (!is.null(time)) {
    ...
    validate(
      need(
        nrow(time) == nrow(data),
        "Numeric-Attribute and Time-Attribute must have conforming IDs (first column). Check these files and reload again;"
      )
    )
  }
