...
#    - Allows users to select training and response datasets (X and Y).
#    - Supports the selection of variables for both independent (X) and
#      dependent (Y) datasets.
#
# 2. Supervised Learning Algorithms:
#    - Integrates with the `caret` package to provide a wide range of
#      machine learning algorithms.
#    - Supports both classification and regression tasks.
#
# 3. Model Training:
#    - Options for training single or multiple models with different target
#      variables or configurations.
#    - Allows partitioning of datasets into training and testing sets.
#    - Supports customization of training parameters and multiple algorithms.
#
# 4. Model Management:
#    - Save, replace, or delete trained models from the selected dataset.
#    - Manage models directly through the interactive UI.
#
# 5. Visualization and Feedback:
#    - Displays information about the training process, including data
#      dimensions and partitioning details.
#    - Provides warnings for potential issues (e.g., missing data).
#
# 6. Dynamic UI and Integration:
#    - Uses dynamic UI components for flexible inputs and outputs.
#    - Integrates with the `caret` and `caretEnsemble` packages for robust
#      model training and evaluation.
#' @export
sl_model_setup<-list()

#' @export
sl_model_setup$ui<-function(id){
  ns<-NS(id)
  div(
        ...
        div(style="display: flex;gap: 10px;",
            ...
            div(style="display: flex;",id=ns("model_model_name"),
                ...
                div(class="save_changes",
                    ...
                    tiphelp_icon(actionButton(ns("save_model"),icon("fas fa-save",style="color: #374061ff")),"Save the model in the current Datalist X", "right")),
                div(style="padding-top: 20px; margin-left: 10px",
                    tiphelp_icon(actionButton(ns("trash_model"),icon("trash")),"Click to remove the model","right"))
            )

        )

      ),
      div(style="margin-left: 20px;margin-top: -2px",align="left",shinyWidgets::dropMenu(actionLink(ns("filter_x"),"+ select columns",style="font-size: 11px;font-style: italic "),uiOutput(ns("filter_var"))))
    ),
    div(
      style="display: flex; gap: 10px; align-items: flex-start",class="setup_box",
      id=ns("model_y_panel"),
      div(
        ...
        div(style="margin-left: 20px;margin-top: -2px",align="left",actionLink(ns("gotrain_loop"),"+ Train multiple models",style="font-size: 11px;font-style: italic "))
      ),
...
#' @export
sl_model_setup$server<-function(id,vals=NULL){
  moduleServer(id,function(input,output,session){

    ns<-session$ns


    train_loop_started<-reactiveVal(FALSE)
    train_loop_modal<-NULL
    start_train_loop_setup<-function(){
      if(isTRUE(train_loop_started())){
        return(NULL)
      }
      ...
      train_loop_started(TRUE)
      NULL
    }
    observeEvent(input$gotrain_loop,{
      start_train_loop_setup()
      req(!is.null(train_loop_modal))
      train_loop_modal()
    },ignoreInit = TRUE)
...
sl_model_params<-list()
sl_model_params$ui <- function(id, vals) {
...
sl_model_params$server<-function(id,vals){
  moduleServer(id,function(input,output,session){
           ...
           box_caret(
             ...
             div(
               div(
                 ...
                 checkboxInput(ns("xyf_varfacmap_action"), span("Variable factor map",actionLink(ns("xyf_varfacmap"), tiphelp_icon(icon("fas fa-question-circle"),'Click for more details',placement ="right"))),value =T),
         ...
         column(4,class="mp0",
                ...
                box_caret(ns('box_b'),
                          ...
                          div(
                            ...
                            numericInput(ns("digits_split"), "Digits split",2, step=1),
                            numericInput(ns("digits_pred"), "Digits pred",1, step=1),
...
msp_rf$server<-function(id,vals){
  moduleServer(id,function(input,output,session){
    predall_rf<-reactiveVal()


    model<-reactive({
      vals$cur_caret_model
    })
    ...
    output$treecm<-renderUI({
      if(model()$modelType=="Classification"){
        ...
        renderPrint(caret::postResample(pred,obc))
      }
    ...
    observe({
      m<-model()
      shinyjs::toggle('fm_palette',condition=m$modelType=="Classification")
      shinyjs::toggle('cforest_norm',condition=m$modelType=="Classification")
      shinyjs::toggle('cm_cell_width',condition=m$modelType=="Classification")
      shinyjs::toggle('msp_plot_base_size',condition=m$modelType=="Classification")
      shinyjs::toggle('fm_round',condition=m$modelType=="Classification")
      shinyjs::toggle('edge_type',condition=m$modelType=="Classification")
      shinyjs::toggle('tree_type',condition=m$modelType=="Classification")
      shinyjs::toggle('cforest_norm',condition=m$modelType=="Classification")


      shinyjs::toggle('digits_split',condition=m$modelType=="Regression")
      shinyjs::toggle('digits_pred',condition=m$modelType=="Regression")
    ...
    gettree<-reactive({
      ...
      if(m$modelType=='Classification')      {
        ...
        plot_rf_tree(m$finalModel,
                     tree_num,
                     digits_split = input$digits_split,
                     digits_pred = input$digits_pred,
                     base_size=input$text_size)
      }
    })
...
sl_custom_grid<-list()
sl_custom_grid$ui<-function(id){
  print("loading custom grid")
  ns<-NS(id)
...
sl_custom_grid$server<-function(id,vals){
  moduleServer(id,function(input,output,session){
      ...
      div(
        column(5,class="mp0",
               box_caret(
                 ...
                 div(
                   pickerInput_fromtop(
                     ns("time_source"),
                     span("Time source:",tipright("Use Numeric-Attribute when the temporal column is also a predictor. Use Temporal-Attribute when the temporal column should only define the validation folds.")),
                     choices = c(
                   ...
                   pickerInput_fromtop(
                     ns("validation_type"),
                     span("Scheme:",tipright("Leave-one-time-block-out CV trains with all other temporal blocks. Prequential temporal CV uses past blocks for training and future blocks for testing, with expanding or rolling windows.")),
                     choices = c(
                       "Leave-one-time-block-out CV" = "time_block_cv",
                       "Prequential temporal CV" = "time_block_prequential"
                     ),
                     selected = "time_block_prequential"
                   ),
                   uiOutput(ns("temporal_scheme_note")),
                   uiOutput(ns("temporal_block_controls")),
                   div(
                     ...
                     pickerInput_fromtop(
                       ns("temporal_window"),
                       span("Temporal window",tipright("Expanding keeps all past blocks and grows the training set through time. Rolling keeps a fixed-size training window that moves forward.")),
                   ...
                   div(
                     ...
                     class="save_changes",
                     style="display: flex; justify-content: flex-end; margin: -2px 0 8px 0;",
                     actionButton(
                       ns("run_temporal_cv"),
                       label = "Create temporal CV scheme",
                       icon = icon("play"),
                       class = "btn-primary",
                       title = "Create temporal CV scheme",
                       style = "white-space: normal; line-height: 1.1; padding: 6px 10px;"
                     )
                   ),
                   div(
                     style="padding: 8px 0 10px 0;",
                     pickerInput_fromtop(
                       ns("temporal_scheme_plot_type"),
                       span("Preview:",tipright("Static keeps the original preview. Interactive uses Plotly with zoom, pan, and tooltips.")),
                       choices=c("Static"="static","Interactive"="plotly"),
                       selected="static"
                     ),
                     uiOutput(ns("temporal_scheme_plot_ui"))
                   )
                 )
               )
        ),
        column(7,class="mp0",
               box_caret(
                 ns("cvt_result_box"),
                 title="Result",
                 button_title=actionLink(ns("down_cvt_scheme_tab"),"Download",icon("download")),
                 div(uiOutput(ns("cvt_result")))
               )
        )
      )
    )
...
temporal_validation$server<-function(id,vals){
  moduleServer(id,function(input,output,session){
    ns<-session$ns

    observe({
      shinyjs::toggle("prequential_args",condition=identical(input$validation_type,"time_block_prequential"))
      shinyjs::toggle("rolling_train_blocks",condition=identical(input$temporal_window,"rolling"))
    })

    has_temporal_attribute<-reactive({
      args<-vals$trainSL_args
      req(args$x_train)
      data_x<-vals$saved_data[[args$data_x]]
      time_attr<-attr(data_x,"time")
      !is.null(time_attr) && (is.data.frame(time_attr) || is.matrix(time_attr)) && ncol(time_attr)>0
    })

    using_fixed_time_blocks<-reactive({
      identical(input$time_source,"time") &&
        isTRUE(has_temporal_attribute()) &&
        identical(input$time_block_mode,"fixed")
    })
    scalar_int_or<-function(x,default){
      x<-suppressWarnings(as.integer(x))
      if(length(x)==0 || !is.finite(x[1]) || is.na(x[1])){
    ...
    get_temporal_vector<-function(){
      ...
      if(identical(input$time_source,"time")){
        time_attr<-attr(data_x,"time")
        validate(need(!is.null(time_attr),"No Temporal-Attribute found in the selected Datalist."))
        validate(need(input$time_var%in%colnames(time_attr),"Selected temporal column was not found in the Temporal-Attribute."))
        validate(need(all(rownames(args$x_train)%in%rownames(time_attr)),"Temporal-Attribute does not contain all training IDs."))
        time_attr[rownames(args$x_train),input$time_var,drop=TRUE]
      } else{
    ...
    parse_temporal_date<-function(x){
      if(inherits(x,"Date")){
        return(as.Date(x))
      }
      ...
      if(any(idx)){
        d[idx]<-suppressWarnings(as.Date(paste0(x_chr[idx],"-01")))
      }
      idx<-is.na(d) & grepl("^[0-9]{4}$",x_chr)
      if(any(idx)){
        d[idx]<-suppressWarnings(as.Date(paste0(x_chr[idx],"-01-01")))
      }
      validate(need(all(!is.na(d)),"Fixed time units require a Date/POSIX temporal attribute or text like YYYY-MM-DD, YYYY-MM, or YYYY."))
      d
    }
    ...
    effective_k_time<-reactive({
      if(isTRUE(using_fixed_time_blocks())){
        blocks<-make_fixed_time_blocks(get_temporal_vector(),input$time_block_unit,input$time_block_width)
        n_blocks<-length(unique(blocks[!is.na(blocks)]))
        if(length(n_blocks)==0 || !is.finite(n_blocks) || is.na(n_blocks) || n_blocks<1){
          return(2L)
        }
        return(as.integer(max(2L,n_blocks)))
      }
    ...
    output$temporal_block_controls<-renderUI({
      if(isTRUE(using_fixed_time_blocks())){
        block_width<-isolate(input$time_block_width)
        if(is.null(block_width) || is.na(block_width) || block_width<1){
          block_width<-1
        }
        block_unit<-get_selected_from_choices(
    ...
    output$estimated_temporal_blocks<-renderUI({
      req(isTRUE(using_fixed_time_blocks()))
      div(style="padding: 6px; background: #f0f7ffff; font-size: 11px",
          div(strong("Estimated temporal blocks: "),effective_k_time()),
          em("For next-month prediction, use Block width = 1, Block unit = Month, Horizon blocks = 1, Step blocks = 1."))
    })

    output$temporal_scheme_note<-renderUI({
      req(input$validation_type)
      if(identical(input$validation_type,"time_block_cv")){
        div(style="padding: 6px; background: #fff8dbff; font-size: 11px",
            em("This scheme does not use expanding or rolling windows; each temporal block is tested once and all other blocks are used for training.")
        )
      } else{
        div(style="padding: 6px; background: #dbf4ffff; font-size: 11px",
            em("Use this scheme for forward validation. The temporal resolution defines the blocks; initial train, horizon, and step define how many splits are generated.")
        )
      }
    })

    output$prequential_split_preview<-renderUI({
      req(input$validation_type=="time_block_prequential")
    ...
    build_temporal_scheme_rows<-reactive({
      req(input$validation_type)
      k_time<-max(2L,scalar_int_or(effective_k_time(),2L))

      if(identical(input$validation_type,"time_block_cv")){
        rows<-lapply(seq_len(k_time),function(i){
          row<-rep("train",k_time)
          row[i]<-"test"
          row
        })
        return(list(
          title="Leave-one-time-block-out CV",
          subtitle=paste0("Temporal blocks = ",k_time,". Each origin tests one block and trains with all other blocks."),
          rows=rows,
          n_blocks=k_time
        ))
      }
      ...
      rows<-lapply(train_end_seq,function(train_end){
        row<-rep("unused",k_time)
        if(identical(input$temporal_window,"rolling")){
          train_start<-train_end-rolling_train_blocks+1L
          if(train_start<1L) train_start<-1L
          train_blocks<-train_start:train_end
        } else{
      ...
      list(
        title=if(identical(input$temporal_window,"rolling")) "Prequential temporal CV: rolling" else "Prequential temporal CV: expanding",
        subtitle=paste0(
    ...
    temporal_scheme_plot_height<-reactive({
      scheme<-build_temporal_scheme_rows()
      n_rows<-length(scheme$rows)
      max(300L,min(1600L,220L+(28L*n_rows)))
    })

    output$temporal_scheme_plot_ui<-renderUI({
      plot_type<-input$temporal_scheme_plot_type
      if(is.null(plot_type)){
        plot_type<-"static"
      }
      if(identical(plot_type,"plotly")){
        plotly::plotlyOutput(ns("temporal_scheme_plotly"),height="430px")
      } else{
        plotOutput(ns("temporal_scheme_plot"),height=paste0(temporal_scheme_plot_height(),"px"))
      }
    })

    output$temporal_scheme_plot<-renderPlot({
      scheme<-build_temporal_scheme_rows()
      rows<-scheme$rows
      n_blocks<-scheme$n_blocks
      n_rows<-length(rows)
      max_visible_rows<-n_rows
      rows_to_plot<-rows
      max_visible_blocks<-min(n_blocks,60L)
      rows_to_plot<-lapply(rows_to_plot,function(row) row[seq_len(max_visible_blocks)])

      par(mar=c(1,1,3,1),family="sans")
      plot.new()
      plot.window(xlim=c(0,1),ylim=c(0,1))
      text(0.06,0.96,scheme$title,adj=c(0,0.5),cex=1.15,font=2)
      text(0.06,0.91,scheme$subtitle,adj=c(0,0.5),cex=0.75,col="#333333")

      x0<-0.08
      x1<-0.94
      block_gap<-0.0015
      row_h<-min(0.055,0.62/max_visible_rows)
      row_gap<-min(0.025,0.24/max_visible_rows)
      start_y<-0.80
      block_w<-(x1-x0)/max_visible_blocks
      cols<-c(train="#0B88A8",test="#FF8A2A",unused="white")

      for(r in seq_along(rows_to_plot)){
        y_top<-start_y-(r-1)*(row_h+row_gap)
        y_bottom<-y_top-row_h
        text(0.04,(y_top+y_bottom)/2,paste0("O",r),cex=0.65,col="#555555")
        for(b in seq_len(max_visible_blocks)){
          state<-rows_to_plot[[r]][b]
          rect(
            x0+(b-1)*block_w+block_gap,y_bottom,
            x0+b*block_w-block_gap,y_top,
            col=cols[[state]],border="black",lwd=0.8
          )
        }
      }
      note_y<-0.13
      if(n_blocks>max_visible_blocks){
        text(0.5,note_y,paste0("Showing first ",max_visible_blocks," of ",n_blocks," temporal blocks"),cex=0.7,col="#555555")
      }
      label_y<-start_y+0.035
      label_every<-max(1L,ceiling(max_visible_blocks/12))
      for(b in seq_len(max_visible_blocks)){
        if(b%%label_every==0 || b==1L || b==max_visible_blocks){
          text(x0+(b-0.5)*block_w,label_y,paste0("T",b),cex=0.55,col="#555555")
        }
      }

      ly<-0.055
      rect(0.27,ly,0.32,ly+0.045,col=cols[["train"]],border="black")
      text(0.335,ly+0.022,"Train",adj=c(0,0.5),cex=0.8)
      rect(0.46,ly,0.51,ly+0.045,col=cols[["test"]],border="black")
      text(0.525,ly+0.022,"Test",adj=c(0,0.5),cex=0.8)
      rect(0.63,ly,0.68,ly+0.045,col=cols[["unused"]],border="black")
      text(0.695,ly+0.022,"Not used",adj=c(0,0.5),cex=0.8)
    },height=function(){temporal_scheme_plot_height()})

    output$temporal_scheme_plotly<-plotly::renderPlotly({
      scheme<-build_temporal_scheme_rows()
      rows<-scheme$rows
      n_blocks<-scheme$n_blocks
      n_rows<-length(rows)
      state_levels<-c(unused=0,train=1,test=2)
      z<-do.call(rbind,lapply(rows,function(row){
        unname(state_levels[row])
      }))
      y_labels<-paste0("O",seq_len(n_rows))
      x_vals<-seq_len(n_blocks)
      hover_text<-matrix("",nrow=n_rows,ncol=n_blocks)
      for(r in seq_len(n_rows)){
        hover_text[r,]<-paste0(
          "Origin: ",y_labels[r],
          "<br>Temporal block: T",x_vals,
          "<br>Status: ",rows[[r]]
        )
      }
      p<-plotly::plot_ly(
      ...
      p<-plotly::layout(
        p,
        title=list(text=paste0(scheme$title,"<br><sup>",scheme$subtitle,"</sup>"),x=0),
        xaxis=list(title="Temporal block",tickmode="auto",nticks=12,fixedrange=FALSE,showgrid=FALSE,zeroline=FALSE),
        yaxis=list(title="Origin",autorange="reversed",fixedrange=FALSE,showgrid=FALSE,zeroline=FALSE),
        dragmode="pan",
        plot_bgcolor="#ffffff",
        paper_bgcolor="#ffffff",
        margin=list(l=55,r=15,t=75,b=45)
      )
      plotly::config(p,scrollZoom=TRUE,displaylogo=FALSE)
    })

    output$time_var_out<-renderUI({
      args<-vals$trainSL_args
      req(args$x_train)
      data_x<-vals$saved_data[[args$data_x]]
      time_source<-input$time_source
      if(is.null(time_source)){
        time_source<-"numeric"
      }
      choices<-if(identical(time_source,"time")){
        time_attr<-attr(data_x,"time")
        if(is.null(time_attr)){
          character(0)
        } else{
          colnames(time_attr)
        }
      } else{
        colnames(args$x_train)
      }
      if(length(choices)==0){
        return(
          div(style="padding: 8px; background: #fff8dbff; font-size: 12px",
              em("No temporal column is available in the selected source.")
          )
        )
      }
      pickerInput_fromtop(
        ns("time_var"),
        span("Time variable:",tipright("Column that defines temporal order or temporal groups.")),
        choices = choices,
        selected = get_selected_from_choices(vals$cur_cvt_time_var, choices),
        options=shinyWidgets::pickerOptions(liveSearch = TRUE, windowPadding="top")
      )
    })

    observeEvent(input$time_var,{
      vals$cur_cvt_time_var<-input$time_var
    })
    observeEvent(input$time_source,{
      vals$cur_cvt_time_source<-input$time_source
      vals$cur_cvt_time_var<-NULL
    })

    build_temporal_df<-reactive({
      args<-vals$trainSL_args
      req(args$x_train)
      req(args$y_train)
      req(input$time_var)
      req(input$time_source)
      data_x<-vals$saved_data[[args$data_x]]
      coords<-attr(data_x,"coords")
      row_ids<-rownames(args$x_train)
      has_coords<-!is.null(coords) &&
        (is.data.frame(coords) || is.matrix(coords)) &&
        ncol(coords)>=2 &&
        !is.null(rownames(coords)) &&
        all(row_ids%in%rownames(coords))
      if(has_coords){
        coords<-coords[row_ids,,drop=FALSE]
        coords_source<-"real"
      } else{
        coords<-data.frame(
          Lon = seq_along(row_ids),
          Lat = rep(0,length(row_ids))
        )
        coords_source<-"auxiliary"
      }
      tempo<-get_temporal_vector()
      if(isTRUE(using_fixed_time_blocks())){
        tempo<-make_fixed_time_blocks(tempo,input$time_block_unit,input$time_block_width)
      ...
      df<-data.frame(
        ...
        Lat = coords[,2],
        Tempo = tempo,
        .response = args$y_train[,1],
        .coords_source = coords_source
      )
      df
    })

    clean_temporal_splits_df<-function(stcv){
      tab<-stcv$splits_df
      if(is.null(tab)){
        return(data.frame())
      }
      spatial_cols<-grep("spatial|fold_sp|spat",names(tab),value=TRUE,ignore.case=TRUE)
      keep_cols<-setdiff(names(tab),spatial_cols)
      tab[,keep_cols,drop=FALSE]
    }

    empty_temporal_result<-function(message="No temporal CV scheme has been created yet. Adjust the parameters and click Create temporal CV scheme."){
      div(style="padding: 10px; font-size: 12px; color: #555555;",
          em(message))
    }

    reset_temporal_scheme<-function(mark_dirty=TRUE){
      vals$cvt<-NULL
    ...
    observeEvent(list(
      ...
      input$k_time,
      input$time_block_size,
      input$time_block_mode,
      input$time_block_width,
    ...
    observeEvent(input$down_cvt_scheme_tab,ignoreInit = TRUE,{
      req(vals$cvt_temporal_splits)
      vals$hand_down<-"generic"
      module_ui_downcenter("downcenter")
      name<-"temporal_cv_scheme"
      mod_downcenter <- callModule(module_server_downcenter, "downcenter",  vals=vals, message="Download Temporal CV Scheme",data=vals$cvt_temporal_splits, name=name)
    })

    observeEvent(input$run_temporal_cv,{
      req(input$validation_type)
      vals$cvt_temporal_splits<-NULL
      time_block_size<-suppressWarnings(as.numeric(input$time_block_size))
      block_size<-if(isTRUE(using_fixed_time_blocks()) || length(time_block_size)==0 || !is.finite(time_block_size[1]) || is.na(time_block_size[1])) NULL else time_block_size[1]
      rolling_blocks<-scalar_int_or(input$rolling_train_blocks,NA_integer_)
      if(is.na(rolling_blocks)) rolling_blocks<-NULL
      res<-tryCatch({
        temporal_df<-build_temporal_df()
        cv_args<-list(
          df = temporal_df,
          spattime_names = c("Lon","Lat","Tempo"),
          response = ".response",
          validation_type = input$validation_type,
          k_time = effective_k_time(),
        ...
        if(!".coords_source"%in%names(cv_obj$data)){
          cv_obj$data$.coords_source<-temporal_df$.coords_source[cv_obj$data$.rowid_original]
        }
        ...
        attr(caret_folds,"params")<-list(
          ...
          k_time = effective_k_time(),
          time_block_size = block_size,
          time_block_mode = if(isTRUE(using_fixed_time_blocks())) "fixed" else "observed",
      ...
      if(inherits(res,"error")){
        ...
        output$cvt_result<-renderUI({
          div(class="alert alert-danger",role="alert",
              ...
              strong("Temporal CV could not be created: "),
              conditionMessage(res))
        })
        shiny::showNotification(
          paste0("Temporal CV could not be created: ", conditionMessage(res)),
          type = "error"
        )
        return(NULL)
      }
      ...
      output$cvt_result<-renderUI({
        ...
        params<-attr(vals$cvt,"params")
        if(is.null(params) || is.null(params$stcv)){
          return(empty_temporal_result())
        }
        stcv<-params$stcv
        tab<-clean_temporal_splits_df(stcv)
        vals$cvt_temporal_splits<-tab
        has_real_coords<-".coords_source"%in%names(stcv$data) &&
          any(as.character(stcv$data$.coords_source)=="real",na.rm=TRUE)
        plot_checker<-if(has_real_coords){
          div(
            style="border-top: 1px solid #dddddd; margin-top: 10px; padding-top: 8px;",
            strong("Check temporal fold"),
            div(
              style="display: flex; gap: 8px; align-items: flex-end; flex-wrap: wrap;",
              div(style="min-width: 150px; flex: 1;",
                  pickerInput_fromtop(
                    ns("cvt_plot_split_id"),
                    span("Fold:",tipright("Select the temporal split that will be shown in the validation plot.")),
                    choices = setNames(tab$split_id,paste0(tab$split_label," | ",tab$time_test_block)),
                    selected = tab$split_id[1]
                  )
              ),
              div(style="min-width: 120px;",
                  pickerInput_fromtop(
                    ns("cvt_plot_type"),
                    span("Plot type:",tipright("Simplified draws a compact validation scheme. Real uses the original spatial coordinates.")),
                    choices = c("Simplified" = "simplified", "Real data" = "real"),
                    selected = "simplified"
                  )
              ),
              actionButton(ns("run_cvt_fold_plot"),"Create plot",icon=icon("chart-line"),class="btn-primary")
            ),
            plotly::plotlyOutput(ns("cvt_fold_plot"),height="360px")
          )
        } else{
          div(style="border-top: 1px solid #dddddd; margin-top: 10px; padding-top: 8px; font-size: 12px; color: #555555;",
              em("No spatial coordinates were found for this Datalist. Use the temporal scheme preview in Parameters to inspect the folds."))
        }
        div(
          div(style="background: #dbf4ffff; padding: 10px; font-size: 12px",
              div(strong("Temporal CV scheme:")),
              div(strong("Scheme:"),params$validation_type),
              div(strong("Time source:"),params$time_source),
          ...
          div(class="half-drop-inline",style="padding-top: 8px; overflow-x: auto",
              fixed_dt(tab,pageLength = 10,dom="ltp",round=NULL,scrollY = "160px",scrollX = "300px",
    ...
    observeEvent(input$run_cvt_fold_plot,{
      req(vals$cvt)
      req(input$cvt_plot_split_id)
      req(input$cvt_plot_type)
      params<-attr(vals$cvt,"params")
      stcv<-params$stcv
      split_id<-as.integer(input$cvt_plot_split_id)
      plot_type<-input$cvt_plot_type
      output$cvt_fold_plot<-plotly::renderPlotly({
        plot_folds(
          stcv,
          split_id = split_id,
          type = plot_type,
          title = paste0("Temporal fold ",input$cvt_plot_split_id),
          verbose = FALSE
        )
      })
    })
...
spatial_size_evaluation<-list()
spatial_size_evaluation$ui<-function(id,cvsp_params){
  ns<-NS(id)
  div(
    column(5,class="mp0",
           box_caret(ns("cvsp_sizeeval_out"),
                     ...
                     div(
                       ...
                       numericInput(ns("cvsp_n_sizes"),span("Number of sizes:",tipright("Number of candidate block sizes to test between the minimum and maximum values. For example, 5 will test five different block sizes.")),value=cvsp_params$n_sizes,min=2,step=1),
                       numericInput(ns("cvsp_size_min"),span("Minimum size:",tipright("Minimum block size to be tested, in metres. Leave empty to let the tool define this value automatically based on the estimated spatial autocorrelation range and study area extent.")),value=cvsp_params$size_min),
                       numericInput(ns("cvsp_size_max"),span("Maximum size:",tipright("Maximum block size to be tested, in metres. Leave empty to let the tool define the largest feasible value based on the number of folds and the approximate number of available spatial blocks.")),value=cvsp_params$size_max),
                       ...
                       numericInput(ns("cvsp_min_blocks_per_fold"),span("Minimum blocks per fold:",tipright("Minimum approximate number of spatial blocks expected per fold. Higher values make the folds more stable, but may force the tool to use smaller block sizes.")),value=cvsp_params$min_blocks_per_fold,min=1,step=1),
                       ...
                       numericInput(ns("cvsp_group2"),span("Groups:",tiphelp("For regression, this defines the number of quantile-based groups used during resampling. The sample is split into groups based on percentiles, and sampling is performed within these groups. This helps preserve the distribution of the response variable across folds.")),value=cvsp_params$group2,min=2,step=1),
                       numericInput(ns("cvsp_iteration2"),span("Iterations:",tipright("Number of random allocation attempts used to find a suitable fold distribution. Larger values may improve fold balance, but increase computation time.")),value=cvsp_params$iteration2,min=1,step=1),
                       numericInput(ns("cvsp_seed2"),span("Seed:",tipright("Random seed used to make the size evaluation reproducible.")),value=cvsp_params$seed2,step=1),
                       div(align="right",actionButton(ns("run_size_eval"),"Run size evaluation",class="btn-primary"))
                     )
           )),
    column(7,class="mp0",
...
spatial_size_evaluation$server<-function(id,vals,cvsp_params,result_cvsp_sizeeval,
                                         best_cvspatial,cvsp_scheme,get_sfdata,y_is_numeric,
                                         apply_label="Use this scheme",
                                         apply_mode="scheme",
                                         on_apply=NULL){
  moduleServer(id,function(input,output,session){
    observe({
    ...
    observe({
      shinyjs::toggle("cvsp_group2",condition=isTRUE(y_is_numeric()))
      shinyjs::toggle("cvsp_iteration2",condition=input$cvsp_selection2=="random")
    })

    observeEvent(input$cvsp_selection2,{
      if(input$cvsp_selection2%in%c("checkerboard","predefined")){
        updateCheckboxInput(session,"cvsp_hexagon2",value=FALSE)
      }
    })
    ...
    observeEvent(input$down_cvsp_sizeeval,{
      ...
      callModule(module_server_figs,"downfigs",vals=vals,generic=generic,message="Size evaluation plot",name_c="size_evaluation")
    })
    ...
    output$cvsp_sizeeval_summary<-DT::renderDT({
      ...
      DT::datatable(
        ...
        width="100%",
        options=list(
          dom="t",paging=FALSE,info=FALSE,ordering=TRUE,autoWidth=FALSE,
          scrollX=TRUE,scrollY="120px",scrollCollapse=TRUE,
          fixedColumns=list(leftColumns=2,rightColumns=0),
          columnDefs=list(list(width="70px",targets=0),list(width="90px",targets=1),list(width="90px",targets=2))
        ),
        callback=DT::JS(
          "setTimeout(function(){",
    ...
    output$cvsp_sizeeval_out<-renderUI({
      req(result_cvsp_sizeeval())
      wars<-attr(result_cvsp_sizeeval(),"warnings")
      errs<-attr(result_cvsp_sizeeval(),"errors")
      div(
        style="padding-top: 10px",
        if(length(result_cvsp_sizeeval())>0) div(strong("Best Block Size: "),result_cvsp_sizeeval()$best_result$size,em("meters")),
        if(!is.null(wars)) div(class="alert_warning",style="padding: 2px",lapply(wars,div)),
        if(!is.null(errs)) div(class="alert alert-danger",role="alert",icon("triangle-exclamation"),lapply(errs,div)),
          ...
          div(
            ...
            div(style="display: flex; gap: 40px",
                div(style="background: #dbf4ffff; padding: 10px; font-size: 12px",
                    ...
                    div(pickerInput(session$ns("cvsp_showrepeat2"),"Show Repeat:",names(result_cvsp_sizeeval()$best_cv_list)))
                )

            ),
            div(align="center",style="padding-top: 20px",actionButton(session$ns("use_this_cvsp"),apply_label),class="save_changes"),
            uiOutput(session$ns("cvsp_evalplot"))
          )
        }
      )
    })
    ...
    observeEvent(input$use_this_cvsp,{
      req(result_cvsp_sizeeval())
      if(identical(apply_mode,"cellsize")){
        req(!is.null(result_cvsp_sizeeval()$best_result$size))
        if(is.function(on_apply)){
          on_apply(result_cvsp_sizeeval()$best_result$size,result_cvsp_sizeeval())
        }
      } else{
        best<-best_cvspatial()
        req(length(best)>0)
    ...
    observeEvent(input$run_size_eval,{
      sf_dat<-get_sfdata()
      cvsp_size_min<-if(is.na(input$cvsp_size_min)) NULL else input$cvsp_size_min
      cvsp_size_max<-if(is.na(input$cvsp_size_max)) NULL else input$cvsp_size_max
      seed<-input$cvsp_seed2
      if(is.na(seed)) seed<-NULL
      if(is.numeric(sf_dat$y)){
        y<-sf_dat$y
        sf_dat$y<-cut(y,unique(quantile(y,probs=seq(0,1,length.out=input$cvsp_group2))),include.lowest=TRUE)
      }
      res_size<-size_evaluation(
        ...
        range_multiplier=c(input$cvsp_range_mult_min,input$cvsp_range_mult_max),
    ...
    output$cvsp_evalplot<-renderUI({
      div(
        renderPlot({
          vals$cvsp_plot02<-cv_plot2(best_cvspatial(),x=get_sfdata(),repeats=input$cvsp_showrepeat2)
          vals$cvsp_plot02
        }),
        DT::renderDT({
          DT::datatable(
            ...
            class="nowrap compact",
            width="100%",
            options=list(dom="t",paging=FALSE,info=FALSE,ordering=TRUE,autoWidth=FALSE,scrollX=TRUE,scrollY="120px",scrollCollapse=TRUE,fixedColumns=list(leftColumns=1,rightColumns=0))
          )
        })
      )
    })
               ...
               box_caret(ns("cvst_parameters"),color="#c3cc74ff",title="Parameters",
                         div(
                           ...
                           div(
                             style="padding: 8px 0 10px 0;",
                             div(
                               style="padding: 6px; margin-bottom: 6px; background: #fff8dbff; font-size: 11px;",
                               uiOutput(ns("spatiotemporal_scheme_preview_note"))
                             ),
                             uiOutput(ns("spatiotemporal_scheme_preview"))
                           )

                         ))
        ),
        column(7,class="mp0",
               box_caret(ns("cvst_result_box"),title="Result",
                         button_title=actionLink(ns("down_cvst_scheme_tab"),"Download",icon("download")),
                         div(uiOutput(ns("cvst_result")))))
      )
    ),
    tabPanel("Cell Size Evaluation",value="cell_size",spatial_size_evaluation$ui(ns("cvst_size_eval"),cvst_params))
  )
}
spatiotemporal_validation$server<-function(id,vals,cvst_params,result_cvst_sizeeval,best_cvst_spatial,cvst_size_scheme,get_sfdata){
  moduleServer(id,function(input,output,session){
    ns<-session$ns
    spatial_size_evaluation$server(
      "cvst_size_eval",
      vals=vals,
      cvsp_params=cvst_params,
      result_cvsp_sizeeval=result_cvst_sizeeval,
      best_cvspatial=best_cvst_spatial,
      cvsp_scheme=cvst_size_scheme,
      get_sfdata=get_sfdata,
      y_is_numeric=reactive({
        !is.null(vals$trainSL_args$y_train) &&
          is.numeric(vals$trainSL_args$y_train[,1])
      }),
      apply_label="Use the suggested cell size",
      apply_mode="cellsize",
      on_apply=function(size,result){
        cvst_params$cellsize<-size
        updateNumericInput(session,"cellsize",value=size)
        updateTabsetPanel(session,"cvst_tabs",selected="create")
      }
    )

    observe({
      shinyjs::toggle("prequential_args",condition=identical(input$validation_type,"spatiotemporal_contiguous_block_prequential"))
      shinyjs::toggle("rolling_train_blocks",condition=identical(input$temporal_window,"rolling"))
    })

    observeEvent(input$validation_type,{
      if(identical(input$validation_type,"spatiotemporal_contiguous_block_prequential")){
        updateNumericInput(session,"k_spat",value=2)
        updateNumericInput(session,"k_time",value=5)
      } else if(identical(input$validation_type,"spatiotemporal_contiguous_block_cv")){
        updateNumericInput(session,"k_spat",value=2)
        updateNumericInput(session,"k_time",value=2)
      }
    },ignoreInit=TRUE)

    output$spatiotemporal_scheme_preview<-renderUI({
      req(input$validation_type)
      preview_file<-if(identical(input$validation_type,"spatiotemporal_contiguous_block_prequential")){
        if(identical(input$temporal_window,"rolling")) "prev_spt_preq_roll.png" else "prev_spt_preq_expand.png"
      } else{
        "prev_spt.png"
      }
      tags$div(
        ...
        tags$img(
          src=paste0("imesc_www/",preview_file),
          style="max-width: 100%; height: auto;"
        )
      )
    })

    output$spatiotemporal_scheme_preview_note<-renderUI({
      req(input$validation_type)
      note<-if(identical(input$validation_type,"spatiotemporal_contiguous_block_prequential")){
        window_label<-if(identical(input$temporal_window,"rolling")) "rolling" else "expanding"
        paste0(
          "The figure below is a static schematic preview of the prequential spatiotemporal CV with a ",
          window_label,
          " temporal window. It changes only with the selected scheme/window;"
        )
      } else{
        "The figure below is a static schematic preview of the spatiotemporal block CV scheme. It changes only with the selected scheme."
      }
      em(note)
    })

    observe({
      if(!is.null(input$k_spat)) cvst_params$k_spat<-input$k_spat
      if(!is.null(input$k_time)) cvst_params$k_time<-input$k_time
      if(!is.null(input$cellsize)) cvst_params$cellsize<-input$cellsize
      if(!is.null(input$grid_shape)) cvst_params$grid_shape<-input$grid_shape
      if(!is.null(input$contiguity)) cvst_params$contiguity<-input$contiguity
    ...
    observeEvent(result_cvst_sizeeval(),{
      req(!is.null(result_cvst_sizeeval()$best_result$size))
      cvst_params$cellsize<-result_cvst_sizeeval()$best_result$size
      updateNumericInput(session,"cellsize",value=result_cvst_sizeeval()$best_result$size)
    })
    ...
    output$time_var_out<-renderUI({
      ...
      req(args$x_train)
      data_x<-vals$saved_data[[args$data_x]]
      choices<-if(identical(input$time_source,"time")){
        time_attr<-attr(data_x,"time")
        if(is.null(time_attr)) character(0) else colnames(time_attr)
      } else{
        colnames(args$x_train)
      }
      if(length(choices)==0){
        return(div(style="padding: 8px; background: #fff8dbff; font-size: 12px",em("No temporal column is available in the selected source.")))
      }
      pickerInput_fromtop(ns("time_var"),span("Time variable:",tipright("Column that defines temporal order or temporal groups.")),choices=choices,selected=get_selected_from_choices(vals$cur_cvst_time_var,choices),options=shinyWidgets::pickerOptions(liveSearch=TRUE,windowPadding="top"))
    })
    ...
    build_cvst_df<-reactive({
      args<-vals$trainSL_args
      req(args$x_train,args$y_train,input$time_var,input$time_source)
      data_x<-vals$saved_data[[args$data_x]]
      coords<-attr(data_x,"coords")
      validate(need(!is.null(coords),"Spatiotemporal CV requires coords-attribute in the X Datalist."))
      row_ids<-rownames(args$x_train)
      validate(need(!is.null(rownames(coords)) && all(row_ids%in%rownames(coords)),"coords-attribute does not contain all training IDs."))
      coords<-coords[row_ids,,drop=FALSE]
      validate(need(ncol(coords)>=2,"coords-attribute must contain at least two coordinate columns."))
      tempo<-if(identical(input$time_source,"time")){
        time_attr<-attr(data_x,"time")
        validate(need(!is.null(time_attr),"No Temporal-Attribute found in the selected Datalist."))
        validate(need(input$time_var%in%colnames(time_attr),"Selected temporal column was not found in the Temporal-Attribute."))
        validate(need(all(row_ids%in%rownames(time_attr)),"Temporal-Attribute does not contain all training IDs."))
      ...
      data.frame(Lon=coords[,1],Lat=coords[,2],Tempo=tempo,.response=args$y_train[,1])
    })

    clean_cvst_splits<-function(stcv){
      tab<-stcv$splits_df
      if(is.null(tab)) data.frame() else tab
    }

    observeEvent(input$down_cvst_scheme_tab,ignoreInit=TRUE,{
      req(vals$cvst_splits)
      vals$hand_down<-"generic"
      module_ui_downcenter("downcenter")
      callModule(module_server_downcenter,"downcenter",vals=vals,message="Download Spatiotemporal CV Scheme",data=vals$cvst_splits,name="spatiotemporal_cv_scheme")
    })

    observeEvent(input$run_cvst,{
      vals$cvst_splits<-NULL
      res<-tryCatch({
        validate(need(!is.na(input$cellsize) && input$cellsize>0,"Cell size must be provided. Use the Cell Size Evaluation tab or enter it manually."))
        cv_args<-list(
          df=build_cvst_df(),
          spattime_names=c("Lon","Lat","Tempo"),
          response=".response",
      ...
      if(inherits(res,"error")){
        ...
        output$cvst_result<-renderUI({
          div(class="alert alert-danger",role="alert",icon("triangle-exclamation"),strong("Spatiotemporal CV could not be created: "),conditionMessage(res))
        })
        shiny::showNotification(paste0("Spatiotemporal CV could not be created: ",conditionMessage(res)),type="error")
        return(NULL)
      }
      ...
      output$cvst_result<-renderUI({
        ...
        stcv<-params$stcv
        tab<-clean_cvst_splits(stcv)
        vals$cvst_splits<-tab
        div(
          div(style="background: #dbf4ffff; padding: 10px; font-size: 12px",
              div(strong("Spatiotemporal CV scheme:")),
              div(strong("Scheme: "),params$validation_type),
          ...
          div(class="half-drop-inline",style="padding-top: 8px; overflow-x: auto",
              fixed_dt(tab,pageLength=10,dom="ltp",round=NULL,scrollY="160px",scrollX="300px",extensions=c("FixedHeader"))),
          div(style="border-top: 1px solid #dddddd; margin-top: 10px; padding-top: 8px;",
              strong("Check fold"),
              div(style="display: flex; gap: 8px; align-items: flex-end; flex-wrap: wrap;",
                  div(style="min-width: 150px; flex: 1;",
                      pickerInput_fromtop(ns("cvst_plot_split_id"),span("Fold:",tipright("Select the spatiotemporal split shown in the plot.")),choices=setNames(tab$split_id,paste0(tab$split_label," | ",tab$spatial_test_fold," | ",tab$time_test_block)),selected=tab$split_id[1])),
                  div(style="min-width: 120px;",
                      pickerInput_fromtop(ns("cvst_plot_type"),span("Plot type:",tipright("Simplified draws a compact validation scheme. Real data uses original coordinates and time.")),choices=c("Simplified"="simplified","Real data"="real"),selected="simplified")),
                  actionButton(ns("run_cvst_fold_plot"),"Create plot",icon=icon("chart-line"),class="btn-primary")
              ),
              plotly::plotlyOutput(ns("cvst_fold_plot"),height="420px")
          )
        )
      })
    })

    observeEvent(input$run_cvst_fold_plot,{
      req(vals$cvst,input$cvst_plot_split_id,input$cvst_plot_type)
      stcv<-attr(vals$cvst,"params")$stcv
      split_id<-as.integer(input$cvst_plot_split_id)
      output$cvst_fold_plot<-plotly::renderPlotly({
        plot_folds(stcv,split_id=split_id,type=input$cvst_plot_type,title=paste0("Spatiotemporal fold ",input$cvst_plot_split_id),verbose=FALSE)
      })
    })
  })
}
sl_validation$ui<-function(id){
  print("loanding sl validation")
...
min-width: 200px
    ...
    div(
      ...
      pickerInput_fromtop(ns("method"),strong("Method:"), choices=list("Repeated Cross-Validation"="repeatedcv",'Boot'="boot","Leave one out"="LOOCV","Leave-group out"="LGOCV","Adaptive CV"="adaptive_cv","Spatial block CV"="spat_cv","Temporal block CV"="time_cv","Spatiotemporal block CV"="st_cv"),
                          options=shinyWidgets::pickerOptions(windowPadding="top")),
      ...
      div(
        uiOutput(ns('cvt_scheme_summary')),
        div(align="center",
            actionButton(ns("temporalValidation_module"),"Create New Temporal CV scheme", style="height: 25px;margin: 2px; margin-left: 0px;padding: 2px;   padding-right: 5px;align-items: center; padding-left:5px; ")
        )

      ),
      div(
        uiOutput(ns('cvst_scheme_summary')),
        div(align="center",
            actionButton(ns("spatiotemporalValidation_module"),"Create New Spatiotemporal CV scheme", style="height: 25px;margin: 2px; margin-left: 0px;padding: 2px;   padding-right: 5px;align-items: center; padding-left:5px; ")
        )

      ),
      pickerInput_fromtop(ns("selfinal"),
                          span("Select Final",
                               tiphelp_icon(actionLink(ns("selfinal_help"),
...
sl_validation$server<-function(id,vals){
  moduleServer(id,function(input,output,session){
    ns<-session$ns
    ...
    cvst_params <- reactiveValues(
      ...
      spatial_mode = "forecast_known_space",
      k2 = 5,
      repeats2 = 1,
      n_sizes = 5,
      size_min = NA,
    ...
    output$spatial_cv_tab2<-renderUI({
      spatial_size_evaluation$ui(ns("cvsp_size_eval"),cvsp_params)
    })
    ...
    cvsp_run_key<-function(){
      paste(
        input$cvsp_k,
        input$cvsp_size,
        input$cvsp_selection,
        input$cvsp_iteration,
        input$cvsp_repeats,
        input$cvsp_seed,
        input$cvsp_hexagon,
        input$cvsp_group,
    ...
    observeEvent(input$run_cv_spatial,{
      ...
      key<-cvsp_run_key()
      cached<-cvsp_scheme_cache()
      if(!is.null(cached$result) && identical(cached$key,key)){
        cvsp_scheme(cached$result)
        ...
        output$cvsp_size_warning<-renderUI({
          div(
            ...
            div(
              ...
              em(paste0(
                "Spatial CV could not be created: ",
                conditionMessage(res),
                " Try reducing the number of folds (k) or using a smaller block size."
              ))
            )
          )
        })
        shiny::showNotification(
          paste0("Spatial CV could not be created: ", conditionMessage(res)),
          type = "error"
        )
        return(NULL)
      }
      ...
      cvsp_scheme_cache(list(key=key,result=res))
      cvsp_plot_cache(new.env(parent=emptyenv()))
      cvsp_scheme(res)
    ...
    observeEvent(input$spatiotemporalValidation_module,{
      start_spatiotemporal_validation()
      showModal(
        modalDialog(
    ...
    best_cvst_spatial<-reactiveVal()
    cvst_size_scheme<-reactiveVal()
    ...
    result_cvst_sizeeval<-reactiveVal()
    get_sfdata<-reactive({
      args<-vals$trainSL_args
      x<-args$x_train
      y<-args$y_train[,1]
    ...
    cvsp_scheme_cache<-reactiveVal(list(key=NULL,result=NULL))
    cvsp_plot_cache<-reactiveVal(new.env(parent=emptyenv()))
    cvsp_scheme_key<-function(scheme){
      if(is.null(scheme) || length(scheme)==0){
        return("none")
      }
      paste(
        scheme[[1]]$k,
        scheme[[1]]$size,
        attr(scheme,"hexagon"),
        attr(scheme,"selection"),
        length(scheme),
        paste(names(scheme),collapse=","),
        sep="|"
      )
    }
    cached_cvsp_plot<-function(name, repeat_id, plot_fun){
      req(repeat_id)
      scheme<-cvsp_scheme()
      req(!is.null(scheme))
      key<-paste(name,cvsp_scheme_key(scheme),repeat_id,sep="|")
      cache<-cvsp_plot_cache()
      if(exists(key,envir=cache,inherits=FALSE)){
        return(get(key,envir=cache,inherits=FALSE))
      }
      plot<-plot_fun()
      assign(key,plot,envir=cache)
      plot
    }
    temporal_validation_started<-reactiveVal(FALSE)
    spatial_size_started<-reactiveVal(FALSE)
    spatiotemporal_validation_started<-reactiveVal(FALSE)

    start_temporal_validation<-function(){
      if(!isTRUE(temporal_validation_started())){
        temporal_validation$server("temporal_validation",vals)
        temporal_validation_started(TRUE)
      }
    }
    start_spatial_validation<-function(){
      if(!isTRUE(spatial_size_started())){
        spatial_size_evaluation$server(
          "cvsp_size_eval",
          vals = vals,
          cvsp_params = cvsp_params,
          result_cvsp_sizeeval = result_cvsp_sizeeval,
          best_cvspatial = best_cvspatial,
          cvsp_scheme = cvsp_scheme,
          get_sfdata = get_sfdata,
          y_is_numeric = reactive({
            !is.null(vals$trainSL_args$y_train) &&
              is.numeric(vals$trainSL_args$y_train[,1])
        ...
        spatial_size_started(TRUE)
      }
    }
    start_spatiotemporal_validation<-function(){
      if(!isTRUE(spatiotemporal_validation_started())){
        spatiotemporal_validation$server(
          "spatiotemporal_validation",
          vals = vals,
          cvst_params = cvst_params,
          result_cvst_sizeeval = result_cvst_sizeeval,
          best_cvst_spatial = best_cvst_spatial,
          cvst_size_scheme = cvst_size_scheme,
          get_sfdata = get_sfdata
        )
        spatiotemporal_validation_started(TRUE)
      }
    }
    observeEvent(input$method,{
      if(identical(input$method,"time_cv")){
        start_temporal_validation()
      }
      if(identical(input$method,"spat_cv")){
        start_spatial_validation()
      }
      if(identical(input$method,"st_cv")){
        start_spatiotemporal_validation()
      }
    },ignoreInit = FALSE)
    ...
    output$cvt_scheme_summary <- renderUI({
      req(input$method=="time_cv")
      if (is.null(vals$cvt)) {
        div(style="padding: 10px; background: #fff8dbff; font-size: 12px;",
            em("No temporal cross-validation scheme has been created yet. Use the 'Create temporal CV scheme' button to generate one.")
        )
      } else{
        params<-attr(vals$cvt,"params")
        stcv<-params$stcv
        div(style="background-color:#dbf4ffff; padding: 10px; font-size: 12px",
            div(strong("Temporal CV scheme:")),
            div(strong("Scheme: "), params$validation_type),
            div(strong("Time source: "), params$time_source),
            div(strong("Time variable: "), params$time_var),
    ...
    output$cvsp_scheme_details_plot<-renderUI({
      div(
        box_caret(ns("cvsp_details01"),
                  ...
                  div(
                    renderPlot({
                      vals$cvsp_plot01<-cached_cvsp_plot(
                        "detail_folds",
                        input$cvsp_detail_showrepeat,
                        function() cv_plot2(cvsp_scheme(),x=get_sfdata(), repeats=input$cvsp_detail_showrepeat)
                      )
                      vals$cvsp_plot01
                    })
                  )
        ...
        box_caret(ns("cvsp_details01tab"),
                  ...
                  div(
                    renderPlot({
                      vals$cvsp_plot01_tab<-cached_cvsp_plot(
                        "detail_heatmap",
                        input$cvsp_detail_showrepeat,
                        function() plot_test_fold_heatmap(cvsp_scheme()[[input$cvsp_detail_showrepeat]]$records)
                      )
    ...
    output$cv_spatial_out1<-renderUI({
      div(
        ...
        box_caret(ns("cvsp_details00"),
                  ...
                  div(
                    renderPlot({
                      vals$cvsp_plot00<-cached_cvsp_plot(
                        "modal_folds",
                        input$cvsp_showrepeat,
                        function() cv_plot2(cvsp_scheme(),x=get_sfdata(),repeats=input$cvsp_showrepeat)
                      )
                      vals$cvsp_plot00
                    })
                  )
        ...
        box_caret(ns("cvsp_details00tab"),
                  ...
                  div(
                    renderPlot({
                      vals$cvsp_plot00_tab<-cached_cvsp_plot(
                        "modal_heatmap",
                        input$cvsp_showrepeat,
                        function() plot_test_fold_heatmap(cvsp_scheme()[[input$cvsp_showrepeat]]$records)
                      )
...
permutation_importance$ui<-function(id){
  ...
  div(
    strong("Permutation importance", tiphelp_icon(actionLink(ns("feaimp_help"),icon("fas fa-question-circle")),"Click for more details", "right")),
  ...
  moduleServer(id,function(input,output,session){
    ...
    cm_source<-reactive({
      ...
      if(input$caret_cm_type=="Resampling"){
        ...
        table(predict(model()$finalModel),model()$trainingData[,'.outcome'])
      }
    })
...
model_results$ui<-function(id){
  ...
  div(
    tabsetPanel(
      ...
      tabPanel(
        "2.2. Performance",value="t2",
        div(
          class="performance_nav_box",
          navlistPanel(
            NULL,
            id=ns("performance_nav"),
            selected="metrics",
            widths=c(2,10),
            tabPanel(
              "Metrics",
              ...
              div(
                class="performance_nav_toolbar half-drop-inline inline_pickers",
            ...
            tabPanel(
              ...
              value="observation",
              div(
                class="performance_nav_actions",
                tiphelp_icon(actionLink(ns('caret_create_errors_train'),span("+ Create Datalist",icon("fas fa-file-signature"))),"Create a datalist with the model training errors", "bottom"),
                tiphelp_icon(actionLink(ns('caret_down_errors_train'),span("+ Download",icon("fas fa-download"),icon("fas fa-table")), style="button_active"),"+ Download Table", "bottom")
              ),
...
model_results$server<-function(id,vals){
  moduleServer(id,function(input,output,session){
    ...
    output$performance_metrics<-renderUI({
      ...
      req(identical(input$performance_nav,"metrics"))
      validate(need(inherits(model(),"train"),"No trained  models found"))
      req(input$round_performace)
      m<-model()

      resampling_table<-m$results[rownames(m$bestTune),,drop=FALSE]
      tune_names<-intersect(colnames(m$bestTune),colnames(resampling_table))
      resampling_metric_names<-setdiff(colnames(resampling_table),tune_names)
      ...
      final_table<-data.frame(rbind(caret::postResample(pred,m$trainingData[,'.outcome'])))

      fmt_value<-function(x){
        if(is.numeric(x)){
          format(round(x,input$round_performace),nsmall=input$round_performace,trim=TRUE)
        } else{
          as.character(x)
        }
      }
      resampling_values<-lapply(resampling_table,fmt_value)
      final_values<-lapply(final_table,fmt_value)
      resampling_col_headers<-lapply(seq_along(colnames(resampling_table)),function(i){
        col_name<-colnames(resampling_table)[i]
        if(length(tune_names)>0 && length(resampling_metric_names)>0 && col_name==resampling_metric_names[1]){
          tags$th(class="metric_group_start",col_name)
        } else{
          tags$th(col_name)
        }
      })
      resampling_col_values<-lapply(seq_along(resampling_values),function(i){
        col_name<-names(resampling_values)[i]
        if(length(tune_names)>0 && length(resampling_metric_names)>0 && col_name==resampling_metric_names[1]){
          tags$td(class="metric_group_start",resampling_values[[i]])
        } else{
          tags$td(resampling_values[[i]])
        }
      })
      final_col_headers<-lapply(seq_along(colnames(final_table)),function(i){
        if(i==1){
          tags$th(class="metric_group_start",colnames(final_table)[i])
        } else{
          tags$th(colnames(final_table)[i])
        }
      })
      final_col_values<-lapply(seq_along(final_values),function(i){
        if(i==1){
          tags$td(class="metric_group_start",final_values[[i]])
        } else{
          tags$td(final_values[[i]])
        }
      })
      ...
      div(
        ...
        tags$table(
          class="table shiny-table table-condensed performance_metrics_table",
          tags$thead(
            tags$tr(
              if(length(tune_names)>0) tags$th(colspan=length(tune_names),""),
              tags$th(class=if(length(tune_names)>0) "metric_group_start",colspan=length(resampling_metric_names),"Resampling"),
              tags$th(class="metric_group_start",colspan=ncol(final_table),"Final model")
            ),
            tags$tr(
              resampling_col_headers,
              final_col_headers
            )
          ),
          tags$tbody(
            tags$tr(
              resampling_col_values,
              final_col_values
            )
          )
        )
    ...
    observation_metrics_cache<-reactiveVal(list(key=NULL,table=NULL))
    observation_metrics_key<-function(m){
      pred_signature<-"no-pred"
      if(!is.null(m$pred)){
        pred_signature<-paste(
          nrow(m$pred),
          paste(colnames(m$pred),collapse=","),
          if("pred"%in%colnames(m$pred))sum(nchar(as.character(m$pred$pred)),na.rm=TRUE) else NA,
          if("obs"%in%colnames(m$pred))sum(nchar(as.character(m$pred$obs)),na.rm=TRUE) else NA,
          if("Resample"%in%colnames(m$pred))sum(nchar(as.character(m$pred$Resample)),na.rm=TRUE) else NA,
          sep="|"
        )
      }
      paste(
        ...
        attr(m,"model_tag"),
        m$modelType,
        paste(unlist(m$bestTune),collapse=","),
        pred_signature,
        sep="|"
      )
    }
    observation_metrics_payload<-reactive({
      req(identical(input$tab2,"t2"))
      req(identical(input$performance_nav,"observation"))
      validate(need(inherits(model(),"train"),"No trained  models found"))
      m<-model()
      observation_key<-observation_metrics_key(m)
      cached_observation<-observation_metrics_cache()
      if(!is.null(cached_observation$table) && identical(cached_observation$key,observation_key)){
        table<-cached_observation$table
      } else{
        table<-accu_rf_class(m)
        observation_metrics_cache(list(key=observation_key,table=table))
      }
    ...
    output$observation_metrics<-renderUI({
      ...
      validate(need(inherits(model(),"train"),"No trained  models found"))
      div(class="half-drop-inline",
          uiOutput(ns("observation_metrics_table")))
    })
    output$observation_metrics_table<-renderUI({
      table<-observation_metrics_payload()
      inline(fixed_dt(
        table,pageLength = 20,dom="ltp",extensions = c("FixedHeader")))
    })

    fold_metrics_cache<-reactiveVal(list(key=NULL,payload=NULL))
    fold_payload_current<-reactiveVal(NULL)
    fold_round_table<-function(tab,digits){
      digits<-suppressWarnings(as.integer(digits))
      if(!is.finite(digits))digits<-2
      digits<-max(0,min(8,digits))
      tab_view<-tab
      round_cols<-vapply(tab_view,is.numeric,logical(1)) & !colnames(tab_view)%in%c("n")
      tab_view[round_cols]<-lapply(tab_view[round_cols],function(x){
        format(round(x,digits),nsmall=digits,trim=TRUE)
      })
      tab_view
    }
    output$fold_metrics<-renderUI({
      req(identical(input$tab2,"t2"))
      req(identical(input$performance_nav,"fold"))
      validate(need(inherits(model(),"train"),"No trained  models found"))
      m<-model()
      validate(need(!is.null(m$pred),"No fold-level predictions found"))
      validate(need(all(c("pred","obs","Resample")%in%colnames(m$pred)),"Fold predictions must include pred, obs and Resample columns"))

      fold_data_signature<-paste(
        nrow(m$pred),
        sum(nchar(as.character(m$pred$pred)),na.rm=TRUE),
        sum(nchar(as.character(m$pred$obs)),na.rm=TRUE),
        sum(nchar(as.character(m$pred$Resample)),na.rm=TRUE),
        paste(head(as.character(m$pred$pred),5),collapse=","),
        paste(tail(as.character(m$pred$pred),5),collapse=","),
        paste(head(as.character(m$pred$obs),5),collapse=","),
        paste(tail(as.character(m$pred$obs),5),collapse=","),
        sep="|"
      )
      fold_payload_key<-paste(
        attr(m,"model_name"),
        attr(m,"model_tag"),
        m$modelType,
        fold_data_signature,
      ...
      cached_fold_payload<-fold_metrics_cache()
      if(!is.null(cached_fold_payload$payload) && identical(cached_fold_payload$key,fold_payload_key)){
        payload<-cached_fold_payload$payload
        list2env(payload,environment())
      } else{
        resample_table<-m$pred
        fold_parts<-strsplit(as.character(resample_table$Resample),"\\.")
        folds<-do.call(rbind,lapply(fold_parts,function(z){
          if(length(z)==1)c(z[1],NA_character_) else z[1:2]
        }))
        folds<-data.frame(Fold=folds[,1],Rep=folds[,2],stringsAsFactors=FALSE)

        resample_table<-cbind(resample_table,folds)
        resample_table$Rep[is.na(resample_table$Rep)]<-"Rep1"

        tune_names<-intersect(colnames(m$bestTune),colnames(resample_table))
        if(length(tune_names)>0){
          keep<-rep(TRUE,nrow(resample_table))
          for(nm in tune_names){
            keep<-keep & resample_table[[nm]]==m$bestTune[[nm]]
          }
          resample_table<-resample_table[keep,,drop=FALSE]
        }
        validate(need(nrow(resample_table)>0,"No fold-level rows found for the selected tuning parameters"))

        fold_natural_levels<-function(x){
          x<-unique(as.character(x))
          last_num<-suppressWarnings(as.numeric(sub("^.*?(\\d+)$","\\1",x)))
          last_num[is.na(last_num)]<-Inf
          prefix<-sub("\\d+$","",x)
          x[order(prefix,last_num,x)]
        }
        fold_levels<-fold_natural_levels(resample_table$Fold)
        rep_levels<-fold_natural_levels(resample_table$Rep)
        resample_table$Fold<-factor(as.character(resample_table$Fold),levels=fold_levels)
        resample_table$Rep<-factor(as.character(resample_table$Rep),levels=rep_levels)

        infer_st_fold_ids<-function(resample){
          resample_chr<-as.character(resample)
          parts<-strsplit(resample_chr,"[._-]")
          fold_sp<-rep(NA_character_,length(parts))
          fold_time<-rep(NA_character_,length(parts))

          for(i in seq_along(parts)){
            p<-parts[[i]]
            sp_candidate<-p[grepl("^S[0-9]+$",p,ignore.case=TRUE)]
            time_candidate<-p[grepl("^T[0-9]+$",p,ignore.case=TRUE)]
            if(length(sp_candidate)>0)fold_sp[i]<-toupper(sp_candidate[1])
            if(length(time_candidate)>0)fold_time[i]<-toupper(time_candidate[1])
          }

          if(any(!is.na(fold_time)) && all(is.na(fold_sp))){
            fold_sp<-rep("S1",length(fold_time))
          }

          data.frame(fold_sp=fold_sp,fold_time=fold_time,stringsAsFactors=FALSE)
        }

        fold_pred_table<-data.frame(
          Observed=resample_table$obs,
          Predicted=resample_table$pred,
          Resample=as.character(resample_table$Resample),
          Fold=as.character(resample_table$Fold),
          Rep=as.character(resample_table$Rep),
          stringsAsFactors=FALSE
        )
        if("rowIndex"%in%colnames(resample_table)){
          fold_pred_table<-cbind(rowIndex=resample_table$rowIndex,fold_pred_table)
        }
        st_fold_ids<-infer_st_fold_ids(resample_table$Resample)
        if(any(!is.na(st_fold_ids$fold_sp))){
          fold_pred_table$fold_sp<-st_fold_ids$fold_sp
        }
        if(any(!is.na(st_fold_ids$fold_time))){
          fold_pred_table$fold_time<-st_fold_ids$fold_time
        }
        if(all(c("fold_sp","fold_time")%in%colnames(fold_pred_table))){
          fold_pred_table$fold_sp_time<-ifelse(
            !is.na(fold_pred_table$fold_sp) & !is.na(fold_pred_table$fold_time),
            paste(fold_pred_table$fold_sp,fold_pred_table$fold_time,sep="-"),
            NA_character_
          )
        }
        fold_pred_order<-intersect(
          c("rowIndex","Observed","Predicted","Resample","Fold","Rep","fold_sp","fold_time","fold_sp_time"),
          colnames(fold_pred_table)
        )
        fold_pred_table<-fold_pred_table[,c(fold_pred_order,setdiff(colnames(fold_pred_table),fold_pred_order)),drop=FALSE]
        ...
        fold_prediction_color<-input$fold_prediction_color
        if(length(fold_prediction_color)==0 || is.null(fold_prediction_color))fold_prediction_color<-"#2c7fb8"
        fold_prediction_base_size<-suppressWarnings(as.numeric(input$fold_prediction_base_size))
        if(length(fold_prediction_base_size)==0)fold_prediction_base_size<-12
        if(!is.finite(fold_prediction_base_size))fold_prediction_base_size<-12
        fold_prediction_base_size<-max(8,min(24,fold_prediction_base_size))
        prediction_theme<-theme_minimal(base_size=fold_prediction_base_size)+
          theme(
            strip.text=element_text(face="bold"),
            plot.title=element_text(face="bold"),
            panel.grid.minor=element_blank()
          )
        fold_plot2_color<-input$fold_plot2_color
        if(length(fold_plot2_color)==0 || is.null(fold_plot2_color))fold_plot2_color<-"#c3cc74ff"
        fold_plot2_text_color<-input$fold_plot2_text_color
        if(length(fold_plot2_text_color)==0 || is.null(fold_plot2_text_color))fold_plot2_text_color<-"black"
        fold_plot3_color<-input$fold_plot3_color
        if(length(fold_plot3_color)==0 || is.null(fold_plot3_color))fold_plot3_color<-"#7bcc7dff"
        fold_plot3_text_color<-input$fold_plot3_text_color
        if(length(fold_plot3_text_color)==0 || is.null(fold_plot3_text_color))fold_plot3_text_color<-"black"
        fold_response_color<-input$fold_response_color
        if(length(fold_response_color)==0 || is.null(fold_response_color))fold_response_color<-"#7bcc7dff"
        fold_spatial_correct_color<-input$fold_spatial_correct_color
        if(length(fold_spatial_correct_color)==0 || is.null(fold_spatial_correct_color))fold_spatial_correct_color<-"#1b9e77"
        fold_spatial_incorrect_color<-input$fold_spatial_incorrect_color
        if(length(fold_spatial_incorrect_color)==0 || is.null(fold_spatial_incorrect_color))fold_spatial_incorrect_color<-"#d95f02"
        fold_plot2_palette<-input$fold_plot2_palette
        if(length(fold_plot2_palette)==0 || is.null(fold_plot2_palette))fold_plot2_palette<-vals$colors_img$val[1]
        if(!fold_plot2_palette%in%names(vals$newcolhabs))fold_plot2_palette<-names(vals$newcolhabs)[1]
        heat_palette<-vals$colors_img$val[grep("heat",vals$colors_img$val,ignore.case=TRUE)]
        if(length(heat_palette)==0)heat_palette<-names(vals$newcolhabs)[grep("heat",names(vals$newcolhabs),ignore.case=TRUE)]
        if(length(heat_palette)==0)heat_palette<-vals$colors_img$val[1]
        fold_plot3_palette<-input$fold_plot3_palette
        if(length(fold_plot3_palette)==0 || is.null(fold_plot3_palette))fold_plot3_palette<-heat_palette[1]
        if(!fold_plot3_palette%in%names(vals$newcolhabs))fold_plot3_palette<-names(vals$newcolhabs)[1]
        fold_response_palette<-input$fold_response_palette
        if(length(fold_response_palette)==0 || is.null(fold_response_palette))fold_response_palette<-vals$colors_img$val[1]
        if(!fold_response_palette%in%names(vals$newcolhabs))fold_response_palette<-names(vals$newcolhabs)[1]
        fold_spatial_palette<-input$fold_spatial_palette
        if(length(fold_spatial_palette)==0 || is.null(fold_spatial_palette))fold_spatial_palette<-"magma"
        if(!fold_spatial_palette%in%names(vals$newcolhabs))fold_spatial_palette<-vals$colors_img$val[1]
        if(!fold_spatial_palette%in%names(vals$newcolhabs))fold_spatial_palette<-names(vals$newcolhabs)[1]
        fold_plot2_pal_fun<-vals$newcolhabs[[fold_plot2_palette]]
        fold_plot3_pal_fun<-vals$newcolhabs[[fold_plot3_palette]]
        fold_response_pal_fun<-vals$newcolhabs[[fold_response_palette]]
        fold_spatial_pal_fun<-vals$newcolhabs[[fold_spatial_palette]]
        fold_plot2_base_size<-suppressWarnings(as.numeric(input$fold_plot2_base_size))
        if(length(fold_plot2_base_size)==0)fold_plot2_base_size<-12
        if(!is.finite(fold_plot2_base_size))fold_plot2_base_size<-12
        fold_plot2_base_size<-max(8,min(24,fold_plot2_base_size))
        fold_plot3_base_size<-suppressWarnings(as.numeric(input$fold_plot3_base_size))
        if(length(fold_plot3_base_size)==0)fold_plot3_base_size<-12
        if(!is.finite(fold_plot3_base_size))fold_plot3_base_size<-12
        fold_plot3_base_size<-max(8,min(24,fold_plot3_base_size))
        fold_response_base_size<-suppressWarnings(as.numeric(input$fold_response_base_size))
        if(length(fold_response_base_size)==0)fold_response_base_size<-12
        if(!is.finite(fold_response_base_size))fold_response_base_size<-12
        fold_response_base_size<-max(8,min(24,fold_response_base_size))
        fold_spatial_base_size<-suppressWarnings(as.numeric(input$fold_spatial_base_size))
        if(length(fold_spatial_base_size)==0)fold_spatial_base_size<-12
        if(!is.finite(fold_spatial_base_size))fold_spatial_base_size<-12
        fold_spatial_base_size<-max(8,min(24,fold_spatial_base_size))
        fold_spatial_point_size<-suppressWarnings(as.numeric(input$fold_spatial_point_size))
        if(length(fold_spatial_point_size)==0)fold_spatial_point_size<-2
        if(!is.finite(fold_spatial_point_size))fold_spatial_point_size<-2
        fold_spatial_point_size<-max(.5,min(8,fold_spatial_point_size))
        fold_spatial_alpha<-suppressWarnings(as.numeric(input$fold_spatial_alpha))
        if(length(fold_spatial_alpha)==0)fold_spatial_alpha<-.85
        if(!is.finite(fold_spatial_alpha))fold_spatial_alpha<-.85
        fold_spatial_alpha<-max(.1,min(1,fold_spatial_alpha))
        fold_spatial_x_breaks<-suppressWarnings(as.integer(input$fold_spatial_x_breaks))
        if(length(fold_spatial_x_breaks)==0)fold_spatial_x_breaks<-3
        if(!is.finite(fold_spatial_x_breaks))fold_spatial_x_breaks<-3
        fold_spatial_x_breaks<-max(2,min(8,fold_spatial_x_breaks))
        fold_spatial_y_breaks<-suppressWarnings(as.integer(input$fold_spatial_y_breaks))
        if(length(fold_spatial_y_breaks)==0)fold_spatial_y_breaks<-4
        if(!is.finite(fold_spatial_y_breaks))fold_spatial_y_breaks<-4
        fold_spatial_y_breaks<-max(2,min(8,fold_spatial_y_breaks))
        fold_spatial_show_base<-input$fold_spatial_show_base
        if(length(fold_spatial_show_base)==0 || is.null(fold_spatial_show_base))fold_spatial_show_base<-TRUE
        plot2_theme<-theme_minimal(base_size=fold_plot2_base_size)+
          theme(
            strip.text=element_text(face="bold"),
            plot.title=element_text(face="bold"),
            panel.grid.minor=element_blank()
          )
        plot3_theme<-theme_minimal(base_size=fold_plot3_base_size)+
          theme(
            strip.text=element_text(face="bold"),
            plot.title=element_text(face="bold"),
            panel.grid.minor=element_blank()
          )
        response_theme<-theme_minimal(base_size=fold_response_base_size)+
          theme(
            strip.text=element_text(face="bold"),
            plot.title=element_text(face="bold"),
            panel.grid.minor=element_blank()
          )
        spatial_theme<-theme_minimal(base_size=fold_spatial_base_size)+
          theme(
            strip.text=element_text(face="bold"),
            plot.title=element_text(face="bold"),
            panel.grid.minor=element_blank(),
        ...
        fold_axis_labels<-function(x){
          labs<-try(adaptative_rounding(x),silent=TRUE)
          if(inherits(labs,"try-error"))labs<-signif(x,7)
          as.character(labs)
        }
        ...
        p6<-NULL
        p7<-NULL
        build_fold_plots<-FALSE
        has_spatial_fold_plot<-FALSE
        metrics_by_fold_rep<-NULL

        if(m$modelType=="Regression"){
          resample_table$error<-resample_table$pred-resample_table$obs
          resample_table$abs_error<-abs(resample_table$error)

          metrics_by_fold<-do.call(rbind,lapply(split(resample_table,resample_table$Fold),function(x){
            met<-caret::postResample(x$pred,x$obs)
            c(
              n=nrow(x),
              met,
              Bias=mean(x$error,na.rm=TRUE),
              ErrorSD=stats::sd(x$error,na.rm=TRUE),
              P90AbsError=as.numeric(stats::quantile(x$abs_error,0.9,na.rm=TRUE))
            )
          }))
          metrics_by_fold<-data.frame(Fold=rownames(metrics_by_fold),metrics_by_fold,row.names=NULL)
          metrics_by_fold<-metrics_by_fold[order(match(as.character(metrics_by_fold$Fold),fold_levels)),,drop=FALSE]
          metrics_by_fold$Fold<-factor(as.character(metrics_by_fold$Fold),levels=fold_levels)
          vals$caret_down_fold_summary<-metrics_by_fold

          perf_fold<-metrics_by_fold
          r2_range<-range(perf_fold$Rsquared,na.rm=TRUE)
          err_range<-range(perf_fold$RMSE,na.rm=TRUE)
          if(!all(is.finite(r2_range)) || diff(r2_range)==0)r2_range<-c(0,1)
          if(!all(is.finite(err_range)) || diff(err_range)==0)err_range<-err_range+c(-0.5,0.5)
          err_to_r2<-function(x){
            (x-err_range[1])/diff(err_range)*diff(r2_range)+r2_range[1]
          }
          r2_to_err<-function(x){
            (x-r2_range[1])/diff(r2_range)*diff(err_range)+err_range[1]
          }
          perf_fold$RMSE_scaled<-err_to_r2(perf_fold$RMSE)
          if(build_fold_plots){
            p7<-ggplot(perf_fold,aes(x=Fold,group=1))+
              geom_line(aes(y=Rsquared,color="R2"),linewidth=.8)+
              geom_point(aes(y=Rsquared,color="R2"),size=2)+
              geom_line(aes(y=RMSE_scaled,color="RMSE"),linewidth=.8)+
              geom_point(aes(y=RMSE_scaled,color="RMSE"),size=2)+
              scale_y_continuous("R2",sec.axis=sec_axis(~r2_to_err(.),name="RMSE"))+
              scale_color_manual(values=c("R2"="#1b9e77","RMSE"="#d95f02"))+
              labs(title="Fold Performance",x="Fold",color=NULL)+
              base_theme+
              theme(legend.position="bottom")

            p1<-ggplot(resample_table,aes(x=obs,y=pred))+
              geom_point(alpha=.65,color=fold_prediction_color)+
              geom_abline(slope=1,intercept=0,linetype=2,color="gray35")+
              facet_wrap(~Fold)+
              labs(title="Observed vs Predicted by Fold",x="Observed",y="Predicted")+
              prediction_theme

            p2<-ggplot(resample_table,aes(x=Fold,y=abs_error))+
              geom_boxplot(fill=fold_plot2_color,color="gray30",outlier.alpha=.6)+
              labs(title="Absolute Error Distribution by Fold",x="Fold",y="Absolute error")+
              plot2_theme

            p3<-ggplot(resample_table,aes(x=Fold,y=error))+
              geom_hline(yintercept=0,linetype=2,color="gray35")+
              geom_boxplot(fill=fold_plot3_color,color="gray30",outlier.alpha=.6)+
              labs(title="Prediction Bias by Fold",x="Fold",y="Prediction error (predicted - observed)")+
              plot3_theme

            p5<-ggplot(resample_table,aes(x=Fold,y=obs))+
              geom_boxplot(fill=fold_response_color,color="gray30",outlier.alpha=.6)+
              labs(title="Observed Response Distribution by Fold",x="Fold",y="Observed response")+
              response_theme
          }

          metrics_by_fold_rep<-do.call(rbind,lapply(split(resample_table,list(resample_table$Fold,resample_table$Rep),drop=TRUE),function(x){
            met<-caret::postResample(x$pred,x$obs)
            data.frame(
              Fold=as.character(x$Fold[1]),
              Rep=as.character(x$Rep[1]),
              n=nrow(x),
              RMSE=unname(met["RMSE"]),
              Rsquared=unname(met["Rsquared"]),
              MAE=unname(met["MAE"]),
              stringsAsFactors=FALSE
            )
          }))
          metrics_by_fold_rep<-metrics_by_fold_rep[order(match(as.character(metrics_by_fold_rep$Fold),fold_levels),match(as.character(metrics_by_fold_rep$Rep),rep_levels)),,drop=FALSE]
          metrics_by_fold_rep$Fold<-factor(as.character(metrics_by_fold_rep$Fold),levels=fold_levels)
          metrics_by_fold_rep$Rep<-factor(as.character(metrics_by_fold_rep$Rep),levels=rep_levels)

          if(build_fold_plots){
            p6<-ggplot(metrics_by_fold_rep,aes(x=Rep,y=Fold,fill=RMSE))+
              geom_tile(color="white")+
              geom_text(aes(label=sprintf("%.2f",RMSE)),size=3)+
              scale_fill_gradient(low="white",high="#d95f02")+
              labs(title="RMSE Stability by Fold and Repetition",x="Repetition",y="Fold",fill="RMSE")+
              base_theme+
              theme(panel.grid=element_blank())
          }

        } else{
          classes<-sort(unique(c(as.character(resample_table$obs),as.character(resample_table$pred))))
          resample_table$obs<-factor(resample_table$obs,levels=classes)
          resample_table$pred<-factor(resample_table$pred,levels=classes)
          resample_table$correct<-resample_table$pred==resample_table$obs

          metrics_by_fold<-do.call(rbind,lapply(split(resample_table,resample_table$Fold),function(x){
            cm<-caret::confusionMatrix(x$pred,x$obs)
            by_class<-cm$byClass
            if(is.null(dim(by_class))){
              bal_acc<-unname(by_class["Balanced Accuracy"])
            } else{
              bal_acc<-by_class[,"Balanced Accuracy",drop=TRUE]
            }
            c(
              n=nrow(x),
              Accuracy=unname(cm$overall["Accuracy"]),
              Kappa=unname(cm$overall["Kappa"]),
              ErrorRate=mean(!x$correct,na.rm=TRUE),
              MeanBalancedAccuracy=mean(bal_acc,na.rm=TRUE)
            )
          }))
          metrics_by_fold<-data.frame(Fold=rownames(metrics_by_fold),metrics_by_fold,row.names=NULL)
          metrics_by_fold<-metrics_by_fold[order(match(as.character(metrics_by_fold$Fold),fold_levels)),,drop=FALSE]
          ...
          perf_long<-rbind(
            data.frame(Fold=metrics_by_fold$Fold,Metric="Accuracy",Value=metrics_by_fold$Accuracy),
            data.frame(Fold=metrics_by_fold$Fold,Metric="Kappa",Value=metrics_by_fold$Kappa)
          )
          perf_long$Fold<-factor(as.character(perf_long$Fold),levels=fold_levels)
          if(build_fold_plots){
            p7<-ggplot(perf_long,aes(x=Fold,y=Value,color=Metric,group=Metric))+
              geom_line(linewidth=.8)+
              geom_point(size=2)+
              coord_cartesian(ylim=c(0,1))+
              scale_color_manual(values=c("Accuracy"="#1b9e77","Kappa"="#386cb0"))+
              labs(title="Fold Performance",x="Fold",y="Metric value",color=NULL)+
              base_theme+
              theme(legend.position="bottom")

            p1<-ggplot(metrics_by_fold,aes(x=Fold,y=Accuracy))+
              geom_col(fill=fold_prediction_color,color="gray30")+
              coord_cartesian(ylim=c(0,1))+
              labs(title="Accuracy by Fold",x="Fold",y="Accuracy")+
              prediction_theme

            confusion_fold<-as.data.frame(table(
              Fold=resample_table$Fold,
              Observed=factor(resample_table$obs,levels=classes),
              Predicted=factor(resample_table$pred,levels=classes)
            ))
            confusion_fold$Observed<-factor(confusion_fold$Observed,levels=rev(classes))
            fold_acc<-aggregate(correct~Fold,data=resample_table,FUN=mean)
            fold_labels<-setNames(
              paste0(fold_acc$Fold," (Acc. ",sprintf("%.2f",fold_acc$correct),")"),
              fold_acc$Fold
            )

            p2<-ggplot(confusion_fold,aes(x=Predicted,y=Observed,fill=Freq))+
              geom_tile(color="white")+
              geom_text(aes(label=Freq),size=3,color=fold_plot2_text_color)+
              facet_wrap(~Fold,strip.position="top",labeller=as_labeller(fold_labels))+
              scale_x_discrete(position="top")+
              scale_fill_gradientn(colors=fold_plot2_pal_fun(100))+
              labs(title="Confusion Matrix by Fold",x="Predicted",y="Observed",fill="Count")+
              plot2_theme+
              theme(
                strip.placement="outside",
                strip.text=element_text(face="bold",size=9),
                axis.title.x.top=element_text(margin=ggplot2::margin(b=6)),
                axis.title.x.bottom=element_blank(),
                axis.line.x.top=element_line(color="gray35",linewidth=.4),
                axis.ticks.x.top=element_line(color="gray35",linewidth=.4),
                axis.line.y.left=element_line(color="gray35",linewidth=.4),
                axis.ticks.y.left=element_line(color="gray35",linewidth=.4),
                axis.ticks.length.x.top=grid::unit(5,"pt"),
                axis.ticks.length.y.left=grid::unit(5,"pt"),
                panel.grid=element_blank()
              )

            class_error_fold<-aggregate(
              incorrect~Fold+obs,
              data=transform(resample_table,incorrect=!correct),
              FUN=mean
            )
            class_error_fold$obs<-factor(class_error_fold$obs,levels=rev(classes))
            show_class_error_labels<-nrow(class_error_fold)<=80
            p3<-ggplot(class_error_fold,aes(x=Fold,y=obs,fill=incorrect))+
              geom_tile(color="white")+
              scale_fill_gradientn(colors=fold_plot3_pal_fun(100),limits=c(0,1),labels=scales::percent_format())+
              labs(title="Class Error Rate by Fold",x="Fold",y="Observed class",fill="Error rate")+
              plot3_theme+
              theme(panel.grid=element_blank())
            if(show_class_error_labels){
              p3<-p3+geom_text(aes(label=sprintf("%.2f",incorrect)),size=3,color=fold_plot3_text_color)
            }

            p5<-ggplot(resample_table,aes(x=Fold,fill=obs))+
              geom_bar(position="fill",color="white")+
              scale_fill_manual(values=fold_response_pal_fun(length(classes)))+
              scale_y_continuous(labels=scales::percent_format())+
              labs(title="Observed Class Composition by Fold",x="Fold",y="Class proportion",fill="Observed class")+
              response_theme
          }
          ...
          metrics_by_fold_rep<-do.call(rbind,lapply(split(resample_table,list(resample_table$Fold,resample_table$Rep),drop=TRUE),function(x){
            data.frame(
              ...
              Rep=as.character(x$Rep[1]),
              n=nrow(x),
              Accuracy=mean(x$pred==x$obs,na.rm=TRUE),
              ErrorRate=mean(x$pred!=x$obs,na.rm=TRUE),
              stringsAsFactors=FALSE
            )
          }))
          metrics_by_fold_rep<-metrics_by_fold_rep[order(match(as.character(metrics_by_fold_rep$Fold),fold_levels),match(as.character(metrics_by_fold_rep$Rep),rep_levels)),,drop=FALSE]
          ...
          if(build_fold_plots){
            p6<-ggplot(metrics_by_fold_rep,aes(x=Rep,y=Fold,fill=Accuracy))+
              geom_tile(color="white")+
              geom_text(aes(label=sprintf("%.2f",Accuracy)),size=3)+
              scale_fill_gradient(low="#d95f02",high="#1b9e77",limits=c(0,1))+
              labs(title="Accuracy Stability by Fold and Repetition",x="Repetition",y="Fold",fill="Accuracy")+
              base_theme+
              theme(panel.grid=element_blank())
          }
        }
        ...
        coords<-NULL
        if(!is.null(xy)){
          coords<-attr(vals$saved_data[[xy$Xdatalist]],"coords")
        }
        if(!is.null(coords) && "rowIndex"%in%colnames(resample_table)){
          has_spatial_fold_plot<-TRUE
          if(build_fold_plots){
            sf_dat<-cbind(vals$saved_data[[xy$Xdatalist]],coords)
            sf_dat<-sf::st_as_sf(sf_dat,coords=colnames(coords))
            pred_sf<-sf_dat[resample_table$rowIndex,]
            pred_sf$Fold<-resample_table$Fold
            pred_sf$obs<-resample_table$obs
            pred_sf$pred<-resample_table$pred

            if(m$modelType=="Regression"){
              pred_sf$error<-resample_table$error
              pred_sf$abs_error<-resample_table$abs_error
              p4<-ggplot(pred_sf)+
                geom_sf(aes(color=abs_error),size=2,alpha=.85)+
                facet_wrap(~Fold)+
                scale_color_viridis_c(option="magma")+
                labs(title="Spatial Distribution of Absolute Error by Fold",color="Abs. error")+
                base_theme
            } else{
              pred_sf$correct<-resample_table$correct
              p4<-ggplot(pred_sf)+
                geom_sf(aes(color=correct),size=2,alpha=.85)+
                facet_wrap(~Fold)+
                scale_color_manual(values=c(`FALSE`="#d95f02",`TRUE`="#1b9e77"))+
                labs(title="Spatial Distribution of Correct and Incorrect Predictions by Fold",color="Correct")+
                base_theme
            }
          }
        }
        ...
        payload<-list(
          ...
          resample_table=resample_table,
          fold_levels=fold_levels,
          rep_levels=rep_levels,
          has_spatial_fold_plot=has_spatial_fold_plot,
          metrics_by_fold=metrics_by_fold,
          metrics_by_fold_rep=metrics_by_fold_rep,
          fold_pred_table=fold_pred_table,
          fold_prediction_color=fold_prediction_color,
          fold_prediction_base_size=fold_prediction_base_size,
          fold_plot2_color=fold_plot2_color,
          fold_plot2_text_color=fold_plot2_text_color,
          fold_plot2_palette=fold_plot2_palette,
          fold_plot2_base_size=fold_plot2_base_size,
          fold_plot3_color=fold_plot3_color,
          fold_plot3_text_color=fold_plot3_text_color,
          fold_plot3_palette=fold_plot3_palette,
          fold_plot3_base_size=fold_plot3_base_size,
          fold_response_color=fold_response_color,
          fold_response_palette=fold_response_palette,
          fold_response_base_size=fold_response_base_size
        )
        fold_metrics_cache(list(key=fold_payload_key,payload=payload))
      }
      fold_payload_current(payload)
      vals$caret_fold_plot1<-payload$p1
      vals$caret_fold_plot2<-payload$p2
      vals$caret_fold_plot3<-payload$p3
      vals$caret_fold_plot4<-payload$p4
      vals$caret_fold_plot5<-payload$p5
      vals$caret_fold_plot6<-payload$p6
      vals$caret_fold_plot7<-payload$p7
      vals$caret_down_fold_summary<-payload$metrics_by_fold
      vals$caret_down_fold_stability<-payload$metrics_by_fold_rep
      vals$caret_down_fold_predictions<-payload$fold_pred_table
      fold_summary_round<-isolate(if(is.null(input$round_fold_summary))2 else input$round_fold_summary)
      fold_stability_round<-isolate(if(is.null(input$round_fold_stability))2 else input$round_fold_stability)
      fold_tab_values<-c("summary","fold_predictions","performance","prediction","diagnostics","response","stability",if(isTRUE(has_spatial_fold_plot))"spatial")
      fold_selected<-isolate(input$fold_tabs)
      if(length(fold_selected)==0 || is.null(fold_selected) || !fold_selected%in%fold_tab_values)fold_selected<-"summary"
      fold_diag_selected<-isolate(input$fold_diag_tabs)
      if(length(fold_diag_selected)==0 || is.null(fold_diag_selected) || !fold_diag_selected%in%c("plot2","plot3"))fold_diag_selected<-"plot2"
      fold_diag_plot2_title<-if(m$modelType=="Regression")"Absolute Error" else "Confusion Matrix"
      fold_diag_plot3_title<-if(m$modelType=="Regression")"Bias" else "Class Error"
      ...
      tabs<-list(
        tabPanel(
          ...
          div(
            class="performance_nav_actions",
            tiphelp_icon(actionLink(ns('caret_down_fold_summary'),span("+ Download",icon("fas fa-download"),icon("fas fa-table")),style="button_active"),"+ Download Table","bottom")
          ),
          div(
            class="fold_table_row",
            div(class="fold_round_control",numericInput(ns("round_fold_summary"),"Round",value=fold_summary_round,min=0,max=8,step=1)),
            div(class="half-drop-inline fold_table_wrap",uiOutput(ns("fold_summary_table")))
          )
        ),
        tabPanel(
          "2.2.2 Fold predictions",
          value="fold_predictions",
          div(
            class="performance_nav_actions",
            tiphelp_icon(actionLink(ns('caret_down_fold_predictions'),span("+ Download",icon("fas fa-download"),icon("fas fa-table")),style="button_active"),"+ Download Table","bottom")
          ),
          div(
            class="half-drop-inline fold_table_wrap",
            uiOutput(ns("fold_predictions_table"))
          )
        ),
        tabPanel(
          "2.2.3 Performance",
          ...
          div(
            class="performance_nav_actions",
            tiphelp_icon(actionLink(ns('down_fold_plot7'),span("Download plot",icon("download")),style="button_active"),"Download plot","bottom")
          ),
          div(class="fold_plot_wrap",plotOutput(ns("fold_plot7"),height="420px"))
        ),
        tabPanel(
          "2.2.4 Prediction",
          value="prediction",
          div(
            class="performance_nav_actions",
            tiphelp_icon(actionLink(ns('down_fold_plot1'),span("Download plot",icon("download")),style="button_active"),"Download plot","bottom")
          ),
          div(
            class="fold_plot_row",
            div(
              class="fold_plot_controls",
              colourpicker::colourInput(ns("fold_prediction_color"),"Color",value=fold_prediction_color,showColour="background",allowTransparent=TRUE),
              numericInput(ns("fold_prediction_base_size"),"Base size",value=fold_prediction_base_size,min=8,max=24,step=1)
            ),
            div(class="fold_plot_wrap",plotOutput(ns("fold_plot1"),height="420px"))
          )
        ),
        tabPanel(if(m$modelType=="Regression")"2.2.5 Error" else "2.2.5 Classes",
                 value="diagnostics",
                 tabsetPanel(
                   NULL,
                   id=ns("fold_diag_tabs"),
                   selected=fold_diag_selected,
                   tabPanel(
                     fold_diag_plot2_title,
                     value="plot2",
                     div(
                       class="fold_plot_row",
                       div(
                         class="fold_plot_controls",
                         if(m$modelType=="Regression"){
                           colourpicker::colourInput(ns("fold_plot2_color"),"Color",value=fold_plot2_color,showColour="background",allowTransparent=TRUE)
                         } else{
                           tagList(
                             pickerInput_fromtop_live(
                               inputId=ns("fold_plot2_palette"),
                               label="Palette",
                               choices=vals$colors_img$val,
                               selected="Blues",
                               options=shinyWidgets::pickerOptions(windowPadding="top"),
                             ...
                             colourpicker::colourInput(ns("fold_plot2_text_color"),"Text",value="black",showColour="background",allowTransparent=TRUE)
                           )
                         },
                         numericInput(ns("fold_plot2_base_size"),"Base size",value=fold_plot2_base_size,min=8,max=24,step=1)
                       ),
                       div(
                         class="fold_plot_wrap",
                         div(
                           class="performance_nav_actions",
                           tiphelp_icon(actionLink(ns('down_fold_plot2'),span("Download plot",icon("download")),style="button_active"),"Download plot","bottom")
                         ),
                         plotOutput(ns("fold_plot2"),height="420px")
                       )
                     )
                   ),
                   tabPanel(
                     fold_diag_plot3_title,
                     value="plot3",
                     div(
                       class="fold_plot_row",
                       div(
                         class="fold_plot_controls",
                         if(m$modelType=="Regression"){
                           colourpicker::colourInput(ns("fold_plot3_color"),"Color",value=fold_plot3_color,showColour="background",allowTransparent=TRUE)
                         } else{
                           tagList(
                             pickerInput_fromtop_live(
                               inputId=ns("fold_plot3_palette"),
                               label="Palette",
                               choices=vals$colors_img$val,
                               selected=fold_plot3_palette,
                               options=shinyWidgets::pickerOptions(windowPadding="top"),
                               choicesOpt=list(content=vals$colors_img$img)
                             ),
                             colourpicker::colourInput(ns("fold_plot3_text_color"),"Text",value=fold_plot3_text_color,showColour="background",allowTransparent=TRUE)
                           )
                         },
                         numericInput(ns("fold_plot3_base_size"),"Base size",value=fold_plot3_base_size,min=8,max=24,step=1)
                       ),
                       div(
                         class="fold_plot_wrap",
                         div(
                           class="performance_nav_actions",
                           tiphelp_icon(actionLink(ns('down_fold_plot3'),span("Download plot",icon("download")),style="button_active"),"Download plot","bottom")
                         ),
                         plotOutput(ns("fold_plot3"),height="420px")
                       )
                     )
                   )
                 )),
        tabPanel(
          ...
          div(
            ...
            div(
              class="fold_plot_controls",
              if(m$modelType=="Regression"){
                colourpicker::colourInput(ns("fold_response_color"),"Color",value=fold_response_color,showColour="background",allowTransparent=TRUE)
              } else{
                pickerInput_fromtop_live(
                  inputId=ns("fold_response_palette"),
                  label="Palette",
                  choices=vals$colors_img$val,
                  selected=fold_response_palette,
                  options=shinyWidgets::pickerOptions(windowPadding="top"),
                  choicesOpt=list(content=vals$colors_img$img)
                )
              },
              numericInput(ns("fold_response_base_size"),"Base size",value=fold_response_base_size,min=8,max=24,step=1)
            ),
            div(
              class="fold_plot_wrap",
              div(
                class="performance_nav_actions",
                tiphelp_icon(actionLink(ns('down_fold_plot5'),span("Download plot",icon("download")),style="button_active"),"Download plot","bottom")
              ),
              plotOutput(ns("fold_plot5"),height="360px")
            )
          )
        ),
        tabPanel("2.2.7 Stability",
                 value="stability",
                 div(
                   class="performance_nav_actions",
                   tiphelp_icon(actionLink(ns('caret_down_fold_stability'),span("+ Download",icon("fas fa-download"),icon("fas fa-table")),style="button_active"),"+ Download Table","bottom")
                 ),
                 div(
                   class="fold_table_row",
                   div(class="fold_round_control",numericInput(ns("round_fold_stability"),"Round",value=fold_stability_round,min=0,max=8,step=1)),
                   div(class="half-drop-inline fold_table_wrap",uiOutput(ns("fold_stability_table")))
                 ),
                 div(
                   class="performance_nav_actions",
                   tiphelp_icon(actionLink(ns('down_fold_plot6'),span("Download plot",icon("download")),style="button_active"),"Download plot","bottom")
                 ),
                 plotOutput(ns("fold_plot6"),height="360px"))
      )
      if(isTRUE(has_spatial_fold_plot)){
        tabs<-c(tabs,list(tabPanel(
          "2.2.8 Spatial",
          value="spatial",
          div(
            class="performance_nav_actions",
            tiphelp_icon(actionLink(ns('down_fold_plot4'),span("Download plot",icon("download")),style="button_active"),"Download plot","bottom")
          ),
          plotOutput(ns("fold_plot4"),height="460px")
        )))
      }

      do.call(tabsetPanel,c(list(NULL,id=ns("fold_tabs"),selected=fold_selected),tabs))
    })

    fold_plot_payload<-reactive({
      payload<-fold_payload_current()
      req(!is.null(payload))
      payload
    })

    fold_build_plot<-function(payload,plot_id){
      m<-payload$m
      resample_table<-payload$resample_table
      metrics_by_fold<-payload$metrics_by_fold
      metrics_by_fold_rep<-payload$metrics_by_fold_rep
      fold_levels<-payload$fold_levels
      rep_levels<-payload$rep_levels
      ...
      numeric_input_value<-function(id,default,min_value,max_value){
        value<-suppressWarnings(as.numeric(input[[id]]))
        if(length(value)==0 || !is.finite(value))value<-default
        max(min_value,min(max_value,value))
      }
      color_input_value<-function(id,default){
        value<-input[[id]]
        if(length(value)==0 || is.null(value))default else value
      }
      palette_input_value<-function(id,default){
        value<-input[[id]]
        if(length(value)==0 || is.null(value))value<-default
        if(!value%in%names(vals$newcolhabs))value<-default
        if(!value%in%names(vals$newcolhabs))value<-names(vals$newcolhabs)[1]
        value
      }
      plot_theme<-function(base_size){
      ...
      if(m$modelType=="Regression"){
        if(plot_id=="p7"){
          ...
          perf_fold$RMSE_scaled<-err_to_r2(perf_fold$RMSE)
          return(ggplot(perf_fold,aes(x=Fold,group=1))+
                   geom_line(aes(y=Rsquared,color="R2"),linewidth=.8)+
                   geom_point(aes(y=Rsquared,color="R2"),size=2)+
                   geom_line(aes(y=RMSE_scaled,color="RMSE"),linewidth=.8)+
                   geom_point(aes(y=RMSE_scaled,color="RMSE"),size=2)+
        ...
        if(plot_id=="p1"){
          fold_prediction_color<-color_input_value("fold_prediction_color","#2c7fb8")
          fold_prediction_base_size<-numeric_input_value("fold_prediction_base_size",12,8,24)
          return(ggplot(resample_table,aes(x=obs,y=pred))+
                   geom_point(alpha=.65,color=fold_prediction_color)+
                   geom_abline(slope=1,intercept=0,linetype=2,color="gray35")+
                   facet_wrap(~Fold)+
                   labs(title="Observed vs Predicted by Fold",x="Observed",y="Predicted")+
                   plot_theme(fold_prediction_base_size))
        }
        if(plot_id=="p2"){
          fold_plot2_color<-color_input_value("fold_plot2_color","#c3cc74ff")
          fold_plot2_base_size<-numeric_input_value("fold_plot2_base_size",12,8,24)
          return(ggplot(resample_table,aes(x=Fold,y=abs_error))+
                   geom_boxplot(fill=fold_plot2_color,color="gray30",outlier.alpha=.6)+
                   labs(title="Absolute Error Distribution by Fold",x="Fold",y="Absolute error")+
                   plot_theme(fold_plot2_base_size))
        }
        if(plot_id=="p3"){
          fold_plot3_color<-color_input_value("fold_plot3_color","#7bcc7dff")
          fold_plot3_base_size<-numeric_input_value("fold_plot3_base_size",12,8,24)
          return(ggplot(resample_table,aes(x=Fold,y=error))+
                   geom_hline(yintercept=0,linetype=2,color="gray35")+
                   geom_boxplot(fill=fold_plot3_color,color="gray30",outlier.alpha=.6)+
                   labs(title="Prediction Bias by Fold",x="Fold",y="Prediction error (predicted - observed)")+
                   plot_theme(fold_plot3_base_size))
        }
        if(plot_id=="p5"){
          fold_response_color<-color_input_value("fold_response_color","#7bcc7dff")
          fold_response_base_size<-numeric_input_value("fold_response_base_size",12,8,24)
          return(ggplot(resample_table,aes(x=Fold,y=obs))+
                   geom_boxplot(fill=fold_response_color,color="gray30",outlier.alpha=.6)+
                   labs(title="Observed Response Distribution by Fold",x="Fold",y="Observed response")+
                   plot_theme(fold_response_base_size))
        }
        if(plot_id=="p6"){
          return(ggplot(metrics_by_fold_rep,aes(x=Rep,y=Fold,fill=RMSE))+
                   geom_tile(color="white")+
                   geom_text(aes(label=sprintf("%.2f",RMSE)),size=3)+
                   scale_fill_gradient(low="white",high="#d95f02")+
                   labs(title="RMSE Stability by Fold and Repetition",x="Repetition",y="Fold",fill="RMSE")+
        ...
        if(plot_id=="p7"){
          ...
          perf_long$Fold<-factor(as.character(perf_long$Fold),levels=fold_levels)
          return(ggplot(perf_long,aes(x=Fold,y=Value,color=Metric,group=Metric))+
                   geom_line(linewidth=.8)+
                   geom_point(size=2)+
                   coord_cartesian(ylim=c(0,1))+
                   scale_color_manual(values=c("Accuracy"="#1b9e77","Kappa"="#386cb0"))+
        ...
        if(plot_id=="p1"){
          fold_prediction_color<-color_input_value("fold_prediction_color","#2c7fb8")
          fold_prediction_base_size<-numeric_input_value("fold_prediction_base_size",12,8,24)
          return(ggplot(metrics_by_fold,aes(x=Fold,y=Accuracy))+
                   geom_col(fill=fold_prediction_color,color="gray30")+
                   coord_cartesian(ylim=c(0,1))+
                   labs(title="Accuracy by Fold",x="Fold",y="Accuracy")+
                   plot_theme(fold_prediction_base_size))
        ...
        if(plot_id=="p2"){
          fold_plot2_palette<-palette_input_value("fold_plot2_palette","Blues")
          fold_plot2_text_color<-color_input_value("fold_plot2_text_color","black")
          fold_plot2_base_size<-numeric_input_value("fold_plot2_base_size",12,8,24)
          confusion_fold<-as.data.frame(table(
            Fold=resample_table$Fold,
            Observed=factor(resample_table$obs,levels=classes),
          ...
          return(ggplot(confusion_fold,aes(x=Predicted,y=Observed,fill=Freq))+
                   geom_tile(color="white")+
                   geom_text(aes(label=Freq),size=3,color=fold_plot2_text_color)+
                   facet_wrap(~Fold,strip.position="top",labeller=as_labeller(fold_labels))+
                   scale_x_discrete(position="top")+
                   scale_fill_gradientn(colors=vals$newcolhabs[[fold_plot2_palette]](100))+
                   labs(title="Confusion Matrix by Fold",x="Predicted",y="Observed",fill="Count")+
                   plot_theme(fold_plot2_base_size)+
                   theme(
                     strip.placement="outside",
        ...
        if(plot_id=="p3"){
          ...
          if(length(heat_palette)==0)heat_palette<-names(vals$newcolhabs)[grep("heat",names(vals$newcolhabs),ignore.case=TRUE)]
          if(length(heat_palette)==0)heat_palette<-vals$colors_img$val[1]
          fold_plot3_palette<-palette_input_value("fold_plot3_palette",heat_palette[1])
          fold_plot3_text_color<-color_input_value("fold_plot3_text_color","black")
          fold_plot3_base_size<-numeric_input_value("fold_plot3_base_size",12,8,24)
          class_error_fold<-aggregate(
            incorrect~Fold+obs,
            data=transform(resample_table,incorrect=!correct),
          ...
          class_error_fold$obs<-factor(class_error_fold$obs,levels=rev(classes))
          p<-ggplot(class_error_fold,aes(x=Fold,y=obs,fill=incorrect))+
            geom_tile(color="white")+
            scale_fill_gradientn(colors=vals$newcolhabs[[fold_plot3_palette]](100),limits=c(0,1),labels=scales::percent_format())+
            labs(title="Class Error Rate by Fold",x="Fold",y="Observed class",fill="Error rate")+
            plot_theme(fold_plot3_base_size)+
            theme(panel.grid=element_blank())
          if(nrow(class_error_fold)<=80){
            p<-p+geom_text(aes(label=sprintf("%.2f",incorrect)),size=3,color=fold_plot3_text_color)
          }
          return(p)
        }
        if(plot_id=="p5"){
          fold_response_palette<-palette_input_value("fold_response_palette",vals$colors_img$val[1])
          fold_response_base_size<-numeric_input_value("fold_response_base_size",12,8,24)
          return(ggplot(resample_table,aes(x=Fold,fill=obs))+
                   geom_bar(position="fill",color="white")+
                   scale_fill_manual(values=vals$newcolhabs[[fold_response_palette]](length(classes)))+
                   scale_y_continuous(labels=scales::percent_format())+
                   labs(title="Observed Class Composition by Fold",x="Fold",y="Class proportion",fill="Observed class")+
                   plot_theme(fold_response_base_size))
        }
        if(plot_id=="p6"){
          return(ggplot(metrics_by_fold_rep,aes(x=Rep,y=Fold,fill=Accuracy))+
                   geom_tile(color="white")+
                   geom_text(aes(label=sprintf("%.2f",Accuracy)),size=3)+
                   scale_fill_gradient(low="#d95f02",high="#1b9e77",limits=c(0,1))+
                   labs(title="Accuracy Stability by Fold and Repetition",x="Repetition",y="Fold",fill="Accuracy")+
      ...
      if(plot_id=="p4" && isTRUE(payload$has_spatial_fold_plot)){
        xy<-attr(m,"inputs")
        coords<-NULL
        if(!is.null(xy)){
          coords<-attr(vals$saved_data[[xy$Xdatalist]],"coords")
      ...
      validate(need(FALSE,"This plot is not available for the selected model."))
    }

    output$fold_summary_table<-renderUI({
      payload<-fold_plot_payload()
      req(payload$metrics_by_fold)
      fold_summary_round<-if(is.null(input$round_fold_summary))2 else input$round_fold_summary
      inline(fixed_dt(fold_round_table(payload$metrics_by_fold,fold_summary_round),pageLength=10,dom="t",extensions=c("FixedHeader")))
    })

    output$fold_stability_table<-renderUI({
      payload<-fold_plot_payload()
      req(payload$metrics_by_fold_rep)
      fold_stability_round<-if(is.null(input$round_fold_stability))2 else input$round_fold_stability
      inline(fixed_dt(fold_round_table(payload$metrics_by_fold_rep,fold_stability_round),pageLength=10,dom="t",extensions=c("FixedHeader")))
    })

    output$fold_predictions_table<-renderUI({
      payload<-fold_plot_payload()
      req(payload$fold_pred_table)
      inline(DT::datatable(
        payload$fold_pred_table,
        rownames=FALSE,
        class="nowrap compact",
        width="100%",
        options=list(dom="ltp",pageLength=20,scrollX=TRUE,autoWidth=FALSE)
      ))
    })

    output$fold_plot1<-renderPlot({p<-fold_build_plot(fold_plot_payload(),"p1");vals$caret_fold_plot1<-p;p})
    output$fold_plot2<-renderPlot({p<-fold_build_plot(fold_plot_payload(),"p2");vals$caret_fold_plot2<-p;p})
    output$fold_plot3<-renderPlot({p<-fold_build_plot(fold_plot_payload(),"p3");vals$caret_fold_plot3<-p;p})
    output$fold_plot4<-renderPlot({p<-fold_build_plot(fold_plot_payload(),"p4");vals$caret_fold_plot4<-p;p})
    output$fold_plot5<-renderPlot({p<-fold_build_plot(fold_plot_payload(),"p5");vals$caret_fold_plot5<-p;p})
    output$fold_plot6<-renderPlot({p<-fold_build_plot(fold_plot_payload(),"p6");vals$caret_fold_plot6<-p;p})
    output$fold_plot7<-renderPlot({p<-fold_build_plot(fold_plot_payload(),"p7");vals$caret_fold_plot7<-p;p})

    observeEvent(ignoreInit = T,input$caret_down_fold_summary,{
      vals$hand_down<-"generic"
      module_ui_downcenter("downcenter")
      name<-paste(attr(model(),"model_tag"),"_","fold_summary")
      mod_downcenter <- callModule(module_server_downcenter, "downcenter", vals=vals, message="Download Fold Summary", data=vals$caret_down_fold_summary, name=name)
    })

    observeEvent(ignoreInit = T,input$caret_down_fold_predictions,{
      vals$hand_down<-"generic"
      module_ui_downcenter("downcenter")
      name<-paste(attr(model(),"model_tag"),"_","fold_predictions")
      mod_downcenter <- callModule(module_server_downcenter, "downcenter", vals=vals, message="Download Fold Predictions", data=vals$caret_down_fold_predictions, name=name)
    })

    observeEvent(ignoreInit = T,input$caret_down_fold_stability,{
      vals$hand_down<-"generic"
      module_ui_downcenter("downcenter")
      name<-paste(attr(model(),"model_tag"),"_","fold_stability")
      mod_downcenter <- callModule(module_server_downcenter, "downcenter", vals=vals, message="Download Fold Stability", data=vals$caret_down_fold_stability, name=name)
    })

    down_fold_plot<-function(plot_id,plot_title,name_suffix){
      vals$hand_plot<-"generic_gg"
      module_ui_figs("downfigs")
      generic<-vals[[plot_id]]
      name_c<-paste(attr(model(),"model_tag"),name_suffix,sep="_")
      datalist_name<-attr(model(),"model_name")
      mod_downcenter<-callModule(module_server_figs,"downfigs",vals=vals,generic=generic,message=plot_title,datalist_name=datalist_name,name_c=name_c)
    }

    observeEvent(ignoreInit = T,input$down_fold_plot1,{
      down_fold_plot("caret_fold_plot1","Fold prediction plot","fold_prediction")
    })

    observeEvent(ignoreInit = T,input$down_fold_plot2,{
      down_fold_plot("caret_fold_plot2","Fold diagnostic plot","fold_diagnostic_1")
    })

    observeEvent(ignoreInit = T,input$down_fold_plot3,{
      down_fold_plot("caret_fold_plot3","Fold diagnostic plot","fold_diagnostic_2")
    })

    observeEvent(ignoreInit = T,input$down_fold_plot4,{
      down_fold_plot("caret_fold_plot4","Fold spatial plot","fold_spatial")
    })

    observeEvent(ignoreInit = T,input$down_fold_plot5,{
      down_fold_plot("caret_fold_plot5","Fold response plot","fold_response")
    })

    observeEvent(ignoreInit = T,input$down_fold_plot6,{
      down_fold_plot("caret_fold_plot6","Fold stability plot","fold_stability")
    })

    observeEvent(ignoreInit = T,input$down_fold_plot7,{
      down_fold_plot("caret_fold_plot7","Fold performance plot","fold_performance")
    })
    ...
    observe({
      if(!isTRUE(input$stack)){
        try(
          updateTextInput(session,"xlab",value=colnames(get_imp())), silent=T)
      }else{
        try(
          updateTextInput(session,"xlab",value='Influence Score'), silent=T
        )
      }
    })
...
model_predic$server<-function(id,vals){
  moduleServer(id,function(input,output,session){
    ...
    output$sl_pred_pal<-renderUI({
      req(model())
      req(model()$modelType=="Classification")
      lab2<-span("+",tiphelp_icon(icon("fas fa-question-circle"),"Confusion Matrix Palette"),"Palette", "right")
...
sl_tuning$server<-function(id,vals){
  moduleServer(id,function(input,output,session){
...
fs$server<-function(id,vals){
  moduleServer(id,function(input,output,session){
    ...
    observeEvent(input$run,ignoreInit = T,{
      ...
      rf_ga <-vals$model_error<-try(withProgress(do.call(caret::gafs.default,vals$args_GA),min=NA,max=NA,message="Running"), silent=T)
...
sl_module$ui<-function(id){
  addResourcePath("imesc_www","inst/www")
  ...
  div(
    ...
    div(class="model_setup",
        box_caret(ns('0'),inline=F,
                  title="Model setup",
                  div(
                    sl_model_setup$ui(ns("sl_model_setup"))

                  ),
        )),
    uiOutput(ns("sl_model_setup_out"))
  )

}
sl_module$server<-function(id,vals){
  moduleServer(id,function(input,output,session){
    ns<-session$ns


    migrate_rf_attrs<-function(datalist_name){
      req(datalist_name%in%names(vals$saved_data))
      rf_models<-attr(vals$saved_data[[datalist_name]],"rf")
      model_names<-names(rf_models)
      ...
      for(model_name in model_names){
        m<-rf_models[[model_name]][[1]]
        if(!identical(attr(m,"Datalist"),datalist_name)){
          attr(m,"Datalist")<-datalist_name
          changed<-TRUE
        }
        if(!identical(attr(m,"model_name"),model_name)){
          attr(m,"model_name")<-model_name
          changed<-TRUE
        }
        rf_models[[model_name]][[1]]<-m
      }
      if(isTRUE(changed)){
        attr(vals$saved_data[[datalist_name]],"rf")<-rf_models
      }
      NULL
    }
    observeEvent(vals$cur_data_sl,{
      migrate_rf_attrs(vals$cur_data_sl)
    },ignoreInit = FALSE)
    output$print_train<-renderUI({
    ...
    install_check_cache<-reactiveVal(list())
    to_install<-reactive({
      req(vals$cmodel)
      cache<-install_check_cache()
      if(vals$cmodel%in%names(cache)){
        return(cache[[vals$cmodel]]$value)
      }
      reqs<- sapply(model_library[[vals$cmodel]],is_installed)
      missing_pkgs<-if(any(reqs=="FALSE")){
        names(reqs[reqs=="FALSE"])
      } else{
        NULL
      }
      cache[[vals$cmodel]]<-list(value=missing_pkgs)
      install_check_cache(cache)
      missing_pkgs
    })
    ...
    tuning_grid_ready<-reactive({
      req(!identical(vals$cmodel,"rfGA"))
      req(vals$cmodel)
      req(vals$cur_caret_search)
      req(vals$cur_xtrain)
      req(vals$cur_var_y)

      if(identical(vals$cur_caret_search,"custom-grid")){
        req(vals$box_caret2_args)
      } else{
        req(vals$box_caret1_args$tuneLength)
        req(nrow(vals$cur_xtrain)>0)
        req(ncol(vals$cur_xtrain)>0)
        req(nrow(vals$cur_var_y)>0)
        req(ncol(vals$cur_var_y)>0)
      }
      TRUE
    })
    output$current_tuning_ui<-renderUI({
      req(isTRUE(tuning_grid_ready()))
    ...
    current_tuning_started<-reactiveVal(FALSE)
    observeEvent(tuning_grid_ready(),{
      if(isTRUE(tuning_grid_ready()) && !isTRUE(current_tuning_started())){
        box_caret_server('6b')
        current_tuning_started(TRUE)
      }
    },ignoreInit = FALSE)
    ...
    model_control_started<-reactiveVal(FALSE)
    model_params_ui_cache<-new.env(parent=emptyenv())
    model_params_ui_cache$key<-NULL
    model_params_ui_cache$ui<-NULL
    start_model_control<-function(){
      if(!isTRUE(model_control_started())){
        sl_model_params$server("arg3",vals)
        model_control_started(TRUE)
      }
    }
    output$sl_model_params<-renderUI({
      req(vals$cmodel)
      req(!identical(vals$cmodel,"rfGA"))
      req(vals$cur_xtrain)
      req(vals$cur_var_y)
      start_model_control()
      y_vec<-vals$cur_var_y[,1]
      y_signature<-paste(class(y_vec),if(is.factor(y_vec)) nlevels(y_vec) else "",sep=":")
      params_key<-paste0(
        vals$cmodel,"|",
        ncol(vals$cur_xtrain),"|",
        y_signature
      )
      if(!identical(model_params_ui_cache$key,params_key)){
        model_params_ui_cache$key<-params_key
        model_params_ui_cache$ui<-sl_model_params$ui(ns("arg3"),vals)
      }
      div(
        model_params_ui_cache$ui,
    ...
    attached_model_libraries<-reactiveVal(character(0))
    observeEvent(vals$cmodel,{
      req(vals$cmodel)
      missing_pkgs<-to_install()
      has_missing<-length(missing_pkgs)>0 && any(missing_pkgs!="NULL")
      req(!isTRUE(has_missing))

      libs<-model_library[[vals$cmodel]]
      libs<-libs[!is.na(libs) & libs!="NULL"]
      req(length(libs)>0)

      attached<-attached_model_libraries()
      libs_to_attach<-setdiff(libs,attached)
      if(length(libs_to_attach)==0){
        return(NULL)
      }

      sapply(libs_to_attach,function(x){
        suppressPackageStartupMessages(library(x,character.only=T))
      })
      attached_model_libraries(unique(c(attached,libs_to_attach)))
      message("attached:",paste(libs_to_attach,collapse=", "))
    ...
    update_run_train_visibility<-function(){
      missing_pkgs<-try(to_install(),silent=TRUE)
      if(inherits(missing_pkgs,"try-error")){
        missing_pkgs<-NULL
      }
      has_missing<-length(missing_pkgs)>0 && any(missing_pkgs!="NULL")
      valid_two_class<-is.null(vals$two_class)|isTRUE(vals$two_class)
      shinyjs::toggle("pkg_installed",condition=has_missing)
      shinyjs::toggle('run_train',condition=valid_two_class && !has_missing)
    }
    observeEvent(vals$two_class,{
      update_run_train_visibility()
    },ignoreInit = FALSE)
    observeEvent(to_install(),{
      update_run_train_visibility()
    },ignoreInit = FALSE)
    observeEvent(input$tab,{
      vals$cur_caret_tab<-input$tab
    },ignoreInit = FALSE)
    ...
    observeEvent(input$install_pkg,{
      ...
      cache<-install_check_cache()
      cache[[vals$cmodel]]<-list(value=NULL)
      install_check_cache(cache)
      attached_model_libraries(setdiff(attached_model_libraries(),missing_pkgs))
    ...
    observeEvent(input$run_train,ignoreInit = T,{
      vals$model_error0<-try({
        ...
        if(args_r$method=='time_cv'){
          validate(need(!is.null(vals$cvt),"Temporal CV scheme not found!"))
          req(!is.null(vals$cvt))
          caret_folds<-vals$cvt
          trControl_list$method<-"cv"
          trControl_list$number<-length(caret_folds$index)
          trControl_list$repeats<-NULL
          trControl_list$p<-NULL
          trControl_list$index <-caret_folds$index
          trControl_list$indexOut <- caret_folds$indexOut
        ...
        if(args_r$method=='st_cv'){
          ...
          req(!is.null(vals$cvst))
          caret_folds<-vals$cvst
          trControl_list$method<-"cv"
          trControl_list$number<-length(caret_folds$index)
          trControl_list$repeats<-NULL
          trControl_list$p<-NULL
        ...
        withProgress(
          ...
          max = ncol(var_y_df),
          {
            for(i in 1:ncol(var_y_df)) {
              ...
              vals$model_error<-m<-try(suppressWarnings({
                set.seed(seed)
                do.call(train,args_train)}), silent=T)
    ...
    sl_model_setup$server("sl_model_setup",vals=vals)
    output$sl_model_setup_out<-renderUI({
               ...
               column(4,class="mp0",
                      ...
                      box_caret(ns('4'),
                                ...
                                div(id=ns('box4_content'),
                                    sl_validation$ui(ns("sl_validation"))
                                ))
               ),
               column(7,class='mp0',
                      uiOutput(ns("training_failed")),
    ...
    sl_tuning$server('sl_tuning',vals)
    output$grid_box_ui<-renderUI({
      req(identical(vals$cur_caret_search,"custom-grid"))

      div(
        id=ns('grid_box'),
        box_caret(ns('2'),
                  div(id=ns('box2_content'),
                      sl_custom_grid$ui(ns("grid")),
                      uiOutput(ns("box2_output"))
                  ))
      )
    })
    custom_grid_started<-reactiveVal(FALSE)
    observeEvent(vals$cur_caret_search,{
      if(identical(vals$cur_caret_search,"custom-grid") && !isTRUE(custom_grid_started())){
        box_caret_server('2')
        sl_custom_grid$server("grid",vals)
        custom_grid_started(TRUE)
      }
    },ignoreInit = FALSE)
    sl_validation$server('sl_validation',vals)
    output$fsga_ui<-renderUI({
      req(identical(vals$cmodel,"rfGA"))
      fs$ui(ns("fsga"))
    })
    fs_started<-reactiveVal(FALSE)
    observeEvent(vals$cmodel,{
      if(identical(vals$cmodel,"rfGA") && !isTRUE(fs_started())){
        fs$server("fsga",vals)
        fs_started(TRUE)
      }
    },ignoreInit = FALSE)
    model_results_started<-reactiveVal(FALSE)
    observeEvent(list(input$tab,vals$update_tab_caret,vals$update_tab_results,vals$cur_caret_model),{
      if(!isTRUE(model_results_started()) &&
         (identical(input$tab,"tab2") ||
          identical(vals$update_tab_caret,"tab2") ||
          !is.null(vals$update_tab_results))){
        model_results$server("caret_results",vals)
        model_results_started(TRUE)
      }
    },ignoreInit = FALSE)
    model_predic_started<-reactiveVal(FALSE)
    observeEvent(list(input$tab,vals$update_tab_caret,vals$cur_caret_model),{
      if(!isTRUE(model_predic_started()) &&
         (identical(input$tab,"tab3") || identical(vals$update_tab_caret,"tab3"))){
        model_predic$server("caret_pred",vals)
        model_predic_started(TRUE)
      }
    },ignoreInit = FALSE)
...
detach_package <- function(pkg) {
  ...
  if (pkg_name %in% search()) {
    tryCatch(
      ...
      error = function(e) {
        message("Erro ao desanexar ", pkg, ": ", conditionMessage(e))
      }
    )
  }
}
