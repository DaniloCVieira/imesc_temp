...
desctools_timeattr$ui<-function(id){
  ...
  div(
    column(
      4,class="mp0",style="max-height: 400px; overflow-y: auto",
      box_caret(
        ns("box_timeattr_a"),
        color="#c3cc74ff",
        title="Options",
        div(
          pickerInput_fromtop_live(ns("time_var"),"Time variable:",choices=NULL),
          numericInput(ns("time_bins"),"Bins:",value=30,min=2,step=1),
          numericInput(ns("time_base_size"),"Base size:",value=12,min=6,step=1),
          colourpicker::colourInput(ns("time_fill"),"Fill:",value="#77AADD",showColour="background",allowTransparent=T),
          colourpicker::colourInput(ns("time_border"),"Border:",value="black",showColour="background",allowTransparent=T),
          textInput(ns("time_title"),"Title:",value="Temporal Attribute Distribution"),
          textInput(ns("time_xlab"),"X lab:",value="Time"),
          textInput(ns("time_ylab"),"Y lab:",value="Number of Observations")
        )
      )
    ),
    column(
      ...
      box_caret(
        ns("box_timeattr_b"),
        title="Plot",button_title=actionLink(ns("downp_timeattr"),"Download",icon("download")),
        div(plotOutput(ns("time_plot"),height="390px"))
      ),
      box_caret(
        ns("box_timeattr_c"),
        title="Summary",
        div(tableOutput(ns("time_summary")))
      )
    )
  )
}
desctools_timeattr$server<-function(id, vals, data){
  moduleServer(id,function(input,output,session){
    box_caret_server("box_timeattr_a")
    box_caret_server("box_timeattr_b")
    box_caret_server("box_timeattr_c")

    time_data<-reactive({
      time<-attr(data(),"time")
      req(time)
      validate(need(ncol(time)>0,"No Time-Attribute found."))
      time
    })

    infer_time_type<-function(x, varname){
      x_no_na<-na.omit(x)
      if(length(x_no_na)==0) return("Empty")

      varname<-tolower(varname)
      x_chr<-as.character(x_no_na)
      x_num<-suppressWarnings(as.numeric(x_no_na))

      if(inherits(x,c("POSIXct","POSIXlt"))) return("Date-Time")
      if(inherits(x,"Date")) return("Date")
      if(all(grepl("^\\d{4}-\\d{2}$", x_chr))) return("Year-Month")
      if(all(grepl("^\\d{4}$", x_chr)) && all(x_num>=1000 & x_num<=3000, na.rm=TRUE)) return("Year")
      if(all(grepl("^\\d{1,2}$", x_chr)) && all(x_num>=1 & x_num<=12, na.rm=TRUE)) return("Month")
      if(all(grepl("^\\d{1,2}:\\d{2}(:\\d{2})?$", x_chr))) return("Time")
      if(grepl("year.*month|month.*year|year_month|month_year|ym", varname) && all(grepl("^\\d{4}[-_/]\\d{1,2}$", x_chr))) return("Year-Month")
      if(grepl("year", varname)) return("Year")
      if(grepl("month", varname)) return("Month")
      if(grepl("date", varname)) return("Date")
      if(is.numeric(x)) return("Temporal index")

      "Temporal label"
    }

    time_summary_df<-reactive({
      time<-time_data()
      data.frame(
        Variable=colnames(time),
        Type=mapply(infer_time_type,time,colnames(time),USE.NAMES=FALSE),
        Class=sapply(time,function(x) paste(class(x),collapse="/")),
        Missing=sapply(time,function(x) sum(is.na(x))),
        Unique=sapply(time,function(x) length(unique(na.omit(x)))),
        Min=sapply(time,function(x) {
          x<-na.omit(x)
          if(length(x)==0) return(NA)
          if(inherits(x,c("Date","POSIXct","POSIXlt")) || is.numeric(x)) as.character(min(x)) else as.character(x[1])
        }),
        Max=sapply(time,function(x) {
          x<-na.omit(x)
          if(length(x)==0) return(NA)
          if(inherits(x,c("Date","POSIXct","POSIXlt")) || is.numeric(x)) as.character(max(x)) else as.character(x[length(x)])
        }),
    ...
    observeEvent(time_data(),{
      choices<-colnames(time_data())
      selected<-get_selected_from_choices(input$time_var,choices)
      updatePickerInput(session,"time_var",choices=choices,selected=selected)
    })

    time_plot<-reactive({
      req(input$time_var,input$time_fill,input$time_border)
      time<-time_data()
      x<-time[[input$time_var]]
      df<-data.frame(time=x)
      unique_n<-length(unique(na.omit(x)))

      if((inherits(x,c("Date","POSIXct","POSIXlt")) || is.numeric(x)) && unique_n>30){
        p<-ggplot(df, aes(x=time))+
          geom_histogram(bins=input$time_bins,fill=input$time_fill,color=input$time_border,na.rm=TRUE)
      } else {
        df$time<-as.factor(df$time)
        p<-ggplot(df, aes(x=time))+
          geom_bar(fill=input$time_fill,color=input$time_border,na.rm=TRUE)+
          theme(axis.text.x=element_text(angle=45,hjust=1))
      }

      p<-p+
        labs(title=input$time_title,x=input$time_xlab,y=input$time_ylab)+
        theme_bw(base_size=input$time_base_size)

      vals$timeattr_plot<-p
      p
    })
    ...
    observeEvent(ignoreInit=T,input$downp_timeattr,{
      ...
      callModule(
        ...
        vals=vals,
        generic=generic,
        message="Time-Attribute plot",
        name_c="time_attribute",
...
desctools_tab1$server<-function(id,vals){
  moduleServer(id,function(input,output,session){
    ...
    desctools_timeattr$server("timeattr",vals,getdata_descX)

    time_tab_inserted<-reactiveVal(FALSE)
    time_attr_available<-reactive({
      time<-attr(getdata_descX(),"time")
      !is.null(time) && ncol(time)>0
    })

    observeEvent(time_attr_available(),{
      has_time<-time_attr_available()

      if(isTRUE(has_time) && !isTRUE(time_tab_inserted())){
        insertTab(
          inputId="summ_options",
          target="3. Factor-Attribute",
          position="after",
          tabPanel(
            value="4. Time-Attribute",
            title="1.4. Time-Attribute",
            desctools_timeattr$ui(ns("timeattr"))
          ),
          select=FALSE,
          session=session
        )
      ...
      if(isFALSE(has_time) && isTRUE(time_tab_inserted())){
        if(identical(input$summ_options,"4. Time-Attribute")){
          updateTabsetPanel(session,"summ_options",selected="Datalist")
        }
        removeTab(
          inputId="summ_options",
          target="4. Time-Attribute",
          session=session
        )
    ...
    factor_level_limit <- 200

    filtered_factors <- reactive({
      factors <- attr(getdata_descX(), "factors")
      req(factors)

      nlevels <- sapply(factors, function(x) length(unique(na.omit(x))))
      keep <- nlevels <= factor_level_limit

      list(
        data = factors[, keep, drop = FALSE],
        excluded = names(nlevels)[!keep],
        nlevels = nlevels
      )
    })

    prepare_ggfactors<-reactive({
      filtered <- filtered_factors()
      if (ncol(filtered$data) == 0) {
        vals$factorsplot <- NULL
      }

      validate(
        need(
          ncol(filtered$data) > 0,
          paste0("Factor plot was not generated: all factor columns have more than ", factor_level_limit, " levels.")
        )
      )

      prepare_factors(filtered$data)
    })
    ...
    output$pfac_levels_warning <- renderUI({
      filtered <- filtered_factors()
      req(length(filtered$excluded) > 0)

      excluded_levels <- filtered$nlevels[filtered$excluded]
      msg <- paste0(
        "Factor plot ignores columns with more than ",
        factor_level_limit,
        " levels. Excluded: ",
        paste(paste0(names(excluded_levels), " (", excluded_levels, ")"), collapse = ", "),
        "."
      )
    ...
    output$factorsplot<-renderUI({
      div(
        uiOutput(ns("pfac_levels_warning")),
        renderPlot(pfactors())
      )
    })
  ...
  column(4,class="mp0",style="height:  calc(100vh - 200px);overflow: auto",
         ...
         box_caret(ns("box_8axes"),title="Score Axes",
                   ...
                   div(
                     ...
                     checkboxInput(ns("show_axis_explain"),"Show Explained Variance",T),
                     checkboxInput(ns("rda_constr"),"Constrained",TRUE)
...
desctools_tab8$server<-function(id,vals){
  moduleServer(id,function(input,output,session){
    ...
    get_rda_model<-reactive({
      ...
      print(paste0("X",input$rda_X))
      print(paste0("Y",input$rda_Y))
    ...
    observe({
      shinyjs::toggle('rda_constr', condition=isTRUE(input$show_axis_explain))
    })
        ...
        column(4,class="mp0",style="height:  calc(100vh - 200px);overflow: auto",
               ...
               box_caret(ns("box_8axes"),title="Score Axes",
                         ...
                         div(
                           ...
                           textInput(ns("ylab"), "ylab:", "RDA II"),
                           checkboxInput(ns("show_axis_explain"),"Show Explained Variance",T),
                           checkboxInput(ns("segrda_constr"),"Constrained",TRUE)
...
desctools_tab9$server<-function(id,vals){
  moduleServer(id,function(input,output,session){
    ...
    observe({
      shinyjs::toggle('segrda_constr', condition=isTRUE(input$show_axis_explain))
    })
