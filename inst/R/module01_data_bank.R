...
databank_module$ui <- function(id) {
  ...
  choices <- paste0("tab", 1:8)
  choices_names <- list(
    div(id = ns("tab1"), tipify_ui(icon("fas fa-archive", style = "color: #333;"), "Numeric-Attribute")),
    div(id = ns("tab2"), icon("fas fa-boxes")),
    div(id = ns("tab3"), icon("fas fa-map-marker-alt")),
    div(id = ns("tab5"), icon("far fa-map")),
    div(id = ns("tab4"), icon("far fa-calendar-alt")),
    div(id = ns("tab6"), icon("fas fa-braille")),
    div(id = ns("tab7"), img(src = sup_icon2, height = "17", width = "20")),
    div(id = ns("tab8"), icon("fas fa-comment"))
  )
  ...
  div(
    div(
      h4(
        "Data Bank",
        class = "imesc_title",
        tiphelp("Visualize, edit, and download data tables using interactive sheets for Numeric, Factor, and Coordinate attributes")
      ),
      box_caret(
        ns("bank_tools"),
      ...
      div(
        div(
          box_caret(
            ...
            title = inline(uiOutput(ns("selected_attr"))),
            button_title = div(
            ...
            div(
              ...
              div(
                ...
                tabsetPanel(
                  ...
                  tabPanel(
                    "tab2",
                    div(style = "background: white;", DT::dataTableOutput(ns("DT_factors")))
                  ),
                  tabPanel(
                    "tab3",
                    div(style = " background: white;", uiOutput(ns("viewcoords")))
                  ),
                  tabPanel(
                    "tab4",
                    div(style = "background: white;", uiOutput(ns("viewtime")))
                  ),
                  tabPanel("tab5", uiOutput(ns("viewshapes"))),
                  tabPanel("tab6", uiOutput(ns("viewsom"))),
                  tabPanel(
                    "tab7",
                    div(
                      div(
                        ...
                        pickerInput_fromtop(
                          ...
                          c(
                            "modelInfo" = "modelInfo",
                            "Resampling" = "resampling|tunning",
                            "finalModel" = "finalModel",
                            "Test" = "partition"
                          ),
                          multiple = TRUE,
                          selected = c("modelInfo", "resampling|tunning", "finalModel", "partition"),
                          width = "350px"
                        ),
                  ...
                  tabPanel("tab8", uiOutput(ns("comments")))
                )
              )
            )
          )
...
databank_module$server <- function(id, vals) {
  moduleServer(id, function(input, output, session) {
    ...
    tab_titles <- c(
      ...
      tab4 = "Temporal-Attribute",
      tab5 = "Shapes-Attribute",
      tab6 = "SOM-Attribute",
      tab7 = "Supervised Models",
      tab8 = "Comments"
    )
    ...
    get_title_attr <- reactive({
      tab_titles[[input$view_datalist]]
    })

    summary_som <- reactive({
      mlist <- attr(getdata_bank(), "som")
      res <- lapply(names(mlist), function(i) supersom_summaries3(mlist[[i]], i))
      res2 <- data.frame(res)
      colnames(res2) <- NULL
      res2
    })
    ...
    get_name_download <- reactive({
      if (identical(input$view_datalist, "tab7")) {
        suffix <- if (identical(input$sl_radiotype, "Classification")) "classification-summary" else "regression-summary"
        return(paste0(input$data_bank, "-", suffix))
      }

      paste0(input$data_bank, "-", get_title_attr())
    })
    ...
    update_imesc_models <- reactive({
      ...
      if (length(models_in) == 0) return(NULL)
    ...
    layers_choices <- reactive({
      ...
      c("Base Shape" = "base_shape", "Layer Shape" = "layer_shape", names(eshape))
    })
    ...
    combsom_down <- reactive({
      res <- do.call("cbind", lapply(attr(getdata_bank(), "som"), train.summary_fun))
      colnames(res) <- names(attr(getdata_bank(), "som"))
      data.frame(res)
    })
    ...
    combsom <- reactive({
      ...
      req(length(soms) > 0)
      res <- do.call("cbind", lapply(soms, train.summary_fun))
      colnames(res) <- names(soms)
      res
    })
    ...
    render_metrics <- function(table, round) {
      ...
      numeric_cols <- which(unlist(lapply(table, is.numeric)))
      table[, numeric_cols] <- round(table[, numeric_cols], round)
      ...
      tables <- DT::formatStyle(
        DT::formatStyle(
          DT::datatable(
            ...
            rownames = TRUE,
            container = container,
            options = list(lengthMenu = list(c(-1), c("All")), info = FALSE, autoWidth = FALSE, dom = "t")
          ),
          c(1),
          `border-left` = "solid 1px"
        ),
        c(ncol_base, ncol(table)),
        `border-right` = "solid 1px"
      )

      div(class = "half-drop-inline", DT::renderDataTable(tables))
    }
    ...
    output$summary_attr <- renderUI({
      ...
      div(
        ...
        div(class = "icon_summary", span("rows:"), uiOutput(ns("nrows"))),
        div(class = "icon_summary", span("columns:"), uiOutput(ns("ncols"))),
        div(class = "icon_summary", span("NAs:"), uiOutput(ns("nas")))
      )
    })
    ...
    output$nrows <- renderUI(div(class = "text_summary", nrow(get_download_data())))
    output$ncols <- renderUI(div(class = "text_summary", ncol(get_download_data())))
    output$nas <- renderUI(div(class = "text_summary", sum(is.na(get_download_data()))))
    output$selected_attr <- renderUI(get_title_attr())

    output$comments_downs <- downloadHandler(
      filename = function() paste(input$data_bank, "_comment", Sys.Date(), ".txt", sep = ""),
    ...
    output$summary_sl <- renderUI({
      ...
      pic2 <- which(grepl("model_tag|model_name", colnames(table)))
      table <- table[, unique(sort(c(pic, pic2)))]

      div(
        tags$style(HTML(
          "
          .sl-table table.dataTable>thead>tr>th { padding: 5px }
    ...
    output$bank_input1 <- renderUI(div())

    output$viewshapes <- renderUI({
      div(
        span(
          inline(pickerInput(ns("pick_elayers"), "Shapes:", layers_choices(), width = "250px")),
    ...
    output$shape_plot <- renderPlot(plot_shape())

    output$ddcogs3 <- renderUI({
      div(class = "ddlinks", actionLink(ns("ddcogs3"), "Edit changes"))
    })

    output$viewsom <- renderUI({
      validate(need(length(attr(getdata_bank(), "som")) > 0, "No SOM model found"))
      div(uiOutput(ns("supersom_summary")))
    })

    output$supersom_summary <- renderUI({
      mlist <- attr(getdata_bank(), "som")
      res_som <- lapply(names(mlist), function(i) supersom_summaries2(mlist[[i]], i))
    ...
    output$comments_remove <- renderUI({
      div(popify(shinyBS::bsButton(ns("comments_remove"), icon("fas fa-trash"), style = "button_active"), NULL, "Delete comment"))
    })

    output$comments_edit <- renderUI(actionButton(ns("comments_edit"), icon("fas fa-edit")))

    output$viewcoords <- renderUI({
      if (is.null(attr(getdata_bank(), "coords"))) {
        div(
          div(div(strong("No coords found in Datalist:", style = "color: red"), em(input$data_bank))),
          uiOutput(ns("add_coords_intru")),
          div(
            style = "margin-top: 20px",
            splitLayout(
              cellWidths = c("30%", "20%"),
              column(12, fileInput(inputId = ns("add_coords_file"), label = NULL)),
              column(12, uiOutput(ns("add_coords_button")))
            )
          )
        )
      } else {
        div(inline(DT::dataTableOutput(ns("DTcoords"))))
      }
    })
    ...
    output$viewbase <- renderUI({
      ...
      column(
        ...
        if (is.null(attr(getdata_bank(), "base_shape"))) {
          fluidRow(
            column(12, p(strong("No base_shape found in Datalist:", style = "color: red"), em(input$data_bank))),
            ...
            column(
              ...
              splitLayout(
                cellWidths = c("30%", "20%"),
                column(12, fileInput(inputId = ns("add_base_file"), label = NULL)),
                column(12, uiOutput(ns("add_base_button")))
              )
            )
          )
        }
    ...
    output$viewlayer <- renderUI({
      ...
      div(
        if (is.null(attr(getdata_bank(), "layer_shape"))) {
          fluidRow(
            column(12, p(strong("No layer_shape found in Datalist:", style = "color: red"), em(input$data_bank))),
            ...
            column(
              ...
              splitLayout(
                cellWidths = c("30%", "20%"),
                column(12, fileInput(inputId = ns("add_layer_file"), label = NULL)),
                column(12, uiOutput(ns("add_layer_button")))
              )
            )
          )
        }
    ...
    output$comment_data <- renderUI(HTML(attr(getdata_bank(), "notes")))
    ...
    output$ddcogs3_pick <- renderUI(span(strong("Datalist:"), em(input$data_bank, style = "color: SeaGreen")))
    ...
    output$ddcogs1_targ <- renderUI(input$ddcogs1_pick)
    output$dfcogs1_targ <- renderUI(input$dfcogs1_pick)
    ...
    output$add_coords_button <- renderUI({
      ...
      actionButton(
        ...
        strong(tipify_ui(icon("fas fa-warehouse"), "Click to save to the Datalist"), "Add", style = "button_active")
      )
    })

    output$add_coords_intru <- renderUI({
      div(
        div(style = "margin-top: 20px", strong("1. Use the button below to upload one.")),
    ...
    output$add_base_intru <- renderUI({
      column(
        12,
        column(12, style = "margin-top: 20px", strong("1. Use the button below to upload one.")),
    ...
    output$split_data_out <- renderUI({
      data <- getdata_bank()
      req(input$col_interval)
      div(class = "picker25", fixed_dt_con$ui(ns("numeric"), data, input$col_interval, label = "Show Group:"))
    })
    ...
    output$DT_data <- DT::renderDataTable(
      {
        ...
        if (ncol(data) > input$col_interval) data <- get_data_split()
        data
      },
      extensions = c("FixedColumns", "FixedHeader"),
      options = dt_opts_fixed,
      rownames = TRUE,
    ...
    output$DTcoords <- DT::renderDataTable(
      data.frame(attr(getdata_bank(), "coords")),
      options = dt_opts_simple,
      rownames = TRUE,
      class = "cell-border compact stripe"
    )

    output$viewtime <- renderUI({
      if (is.null(attr(getdata_bank(), "time"))) {
        div(
          div(
            strong("No Temporal-Attribute found in Datalist:", style = "color: red"),
            em(input$data_bank)
          )
        )
      } else {
        div(inline(DT::dataTableOutput(ns("DT_time"))))
      }
    })

    output$DT_time <- DT::renderDataTable(
      {
        data <- attr(getdata_bank(), "time")
        validate(need(!is.null(data), "No Temporal-Attribute found in this Datalist."))
        data.frame(data)
      },
      options = dt_opts_simple,
      rownames = TRUE,
      ...
      modalDialog(
        ...
        div(
          ...
          span(
            ...
            shinyBS::bsButton(ns("ddcogs1_rename_remove"), icon(verify_fa = FALSE, name = NULL, class = "fas fa-trash-alt")),
            modalButton("Dismiss")
          ),
          div(renderPrint(attr(getdata_bank(), "transf")))
        )
      )
    })
    ...
    modal_comment <- reactive({
      modalDialog(
        ...
        easyClose = TRUE,
        footer = div(modalButton("Cancel"), actionButton(ns("confirm_note"), "3. Confirm")),
        div(uiOutput(ns("note_target")), uiOutput(ns("note_create")))
      )
    })
    ...
    output$note_target <- renderUI({
      tags$div(
        ...
        pickerInput(ns("note_targ"), "1. Select the target Datalist:", names(vals$saved_data), selected = vals$cur_data)
      )
    })
    ...
    output$note_create <- renderUI({
      ...
      textAreaInput(
        ns("note_new"),
        "2. Comments:",
        value = gsub("<br/>", "\n", attr(vals$saved_data[[input$note_targ]], "notes")),
        width = "500px",
        height = "150px",
        cols = NULL,
        rows = NULL,
    ...
    observe({
      shinyjs::toggle("sl_radiotype", condition = input$view_datalist == "tab7")
    })

    observe({
      shinyjs::toggle("summary_attr", condition = input$view_datalist %in% c("tab1", "tab2", "tab3", "tab4"))
    })

    observe({
      shinyjs::toggle("download_table", condition = input$view_datalist %in% c("tab1", "tab2", "tab3", "tab4", "tab6", "tab7"))
      shinyjs::toggle("download_plot", condition = input$view_datalist %in% c("tab5"))
      shinyjs::toggle("comments_downs", condition = input$view_datalist %in% c("tab8"))
    })
    ...
    observeEvent(ignoreInit = TRUE, input$download_table, {
      vals$hand_down <- "generic"
      module_ui_downcenter("downcenter")
      data <- get_download_data()
      name <- get_name_download()
      callModule(module_server_downcenter, "downcenter", vals = vals, data = data, name = name)
    })

    observeEvent(ignoreInit = TRUE, input$download_plot, {
      vals$hand_plot <- "generic_gg"
      generic <- plot_shape()
      message <- name_c <- "Shape"
      module_ui_figs("downfigs")
      datalist_name <- attr(getdata_bank(), "datalist")
      callModule(module_server_figs, "downfigs", vals = vals, message = message, name_c = name_c, generic = generic, datalist_name = datalist_name)
    })
    ...
    observeEvent(input$data_bank, {
      ...
      choices_names <- list(
        ...
        div(id = ns("tab4"), icon("far fa-calendar-alt"), cogs_title = "Temporal-Attribute"),
        div(id = ns("tab5"), icon("far fa-map"), cogs_title = "Shapes-Attribute"),
        div(id = ns("tab6"), icon("fas fa-braille"), cogs_title = "SOM-Attribute"),
        div(id = ns("tab7"), img(src = sup_icon2, height = "17", width = "20"), cogs_title = "Supervised Models"),
        div(id = ns("tab8"), icon("fas fa-comment"), cogs_title = "Comments")
      )

      somyes <- length(attr(getdata_bank(), "som")) > 0
      supyes <- any(sapply(get_metrics(), length) > 0)
      timeyes <- !is.null(attr(getdata_bank(), "time")) && ncol(attr(getdata_bank(), "time")) > 0

      for (tab in c(if (isFALSE(somyes)) "tab6", if (isFALSE(supyes)) "tab7", if (isFALSE(timeyes)) "tab4")) {
        rem <- which(choices == tab)
        choices <- choices[-rem]
        choices_names[rem] <- NULL
      }

      selected <- get_selected_from_choices(vals$cur_view_datalist, choices)
      vals$cur_view_datalist <- selected
      vals$curview_databank <- selected
      updateRadioGroupButtons(session, "view_datalist", choiceValues = choices, choiceNames = choices_names, selected = selected)
      updateTabsetPanel(session, "tab_bank", selected = selected)
      ...
      shinyBS::addPopover(session, "tab4", NULL, "Temporal-Attribute")
      shinyBS::addPopover(session, "tab5", NULL, "Shapes-Attribute")
      shinyBS::addPopover(session, "tab6", NULL, "SOM-Attribute")
      shinyBS::addPopover(session, "tab7", NULL, "Supervised Models")
      shinyBS::addPopover(session, "tab8", NULL, "Comments")
    })

    observeEvent(ignoreInit = TRUE, input$data_bank, {
      vals$cur_data <- input$data_bank
    })

    observeEvent(ignoreInit = TRUE, input$view_datalist, {
    ...
    observeEvent(ignoreInit = TRUE, input$DT_data_cell_edit, {
    ...
    observeEvent(ignoreInit = TRUE, input$DT_factors_cell_edit, {
      factors <- attr(getdata_bank(), "factors")
      ...
      if (!value %in% levels(factors[, clmn])) {
        ...
        attr(vals$saved_data[[input$data_bank]], "factors")[row, clmn] <- value
      }
    })

    observeEvent(ignoreInit = TRUE, input$delete_coords, {
      showModal(
        modalDialog(
          title = "Are you sure?",
          column(
            ...
            shinyBS::bsButton(ns("delete_coords_yes"), icon = icon(verify_fa = FALSE, name = NULL, class = "far fa-trash-alt"), "Confirm", style = "button_active", block = TRUE)
          ),
          easyClose = TRUE,
          size = "s"
        )
      ...
      showModal(
        modalDialog(
          ...
          div(
            div(
              div(
                style = "overflow-y: scroll;height: 200px;overflow-x: scroll; padding-top: 10px",
                checkboxGroupInput(ns("remove_som"), NULL, choices = names(attr(getdata_bank(), "som")))
              )
            ),
            shinyBS::bsButton(ns("delete_som"), icon = icon(verify_fa = FALSE, name = NULL, class = "far fa-trash-alt"), "Delete", style = "button_active", block = TRUE, value = FALSE)
          )
        )
      )
    })

    observeEvent(ignoreInit = TRUE, input$delete_som, {
      showModal(
        modalDialog(
          title = "Are you sure?",
          column(
            ...
            shinyBS::bsButton(ns("delete_som_yes"), icon = icon(verify_fa = FALSE, name = NULL, class = "far fa-trash-alt"), "Confirm", style = "button_active", block = TRUE)
          ),
          easyClose = TRUE,
          size = "s"
        )
    ...
    observeEvent(ignoreInit = TRUE, input$delete_som_yes, {
      attr(vals$saved_data[[input$data_bank]], "som")[input$remove_som] <- NULL
      removeModal()
    })

    observeEvent(ignoreInit = TRUE, input$add_base, {
      t <- try({
        get(gsub(" ", "", capture.output(load(input$add_base_file$datapath, verbose = TRUE))[2]))
      })
    ...
    observeEvent(ignoreInit = TRUE, input$add_layer, {
      t <- try({
        get(gsub(" ", "", capture.output(load(input$add_layer_file$datapath, verbose = TRUE))[2]))
      })
    ...
    observeEvent(ignoreInit = TRUE, input$add_coords_file, {
      output$error_coords <- renderUI(NULL)
    })

    observeEvent(ignoreInit = TRUE, input$add_coords, {
      coords <- data.frame(data.table::fread(input$add_coords_file$datapath))
      rownames(coords) <- coords[, 1]
      coords[, 1] <- NULL

      if (ncol(coords) != 2) {
        output$error_coords <- renderUI({
          div(strong(
            "Invalid Entries. The first column must contain the name of the observations. The second and third columns must contain the logitude and latitude respectively",
            style = "color: red"
          ))
        })
      }

      if (any(rownames(coords) %in% rownames(getdata_bank())) == FALSE) {
        output$error_coords <- renderUI({
          div(strong(
            "None of the IDs of the banked coordinates are compatible with the ids of the selected datalist. Please bank coodinates with valid IDs",
            style = "color: red"
          ))
        })
      }
      ...
      attr(vals$saved_data[[input$data_bank]], "coords") <- na.omit(coords[rownames(getdata_bank()), ])
      updateRadioGroupButtons(session, "view_datalist", selected = "coords")
    })

    observeEvent(ignoreInit = TRUE, input$delete_elayer_shape, {
      req(input$pick_elayers)

      if (identical(input$pick_elayers, "base_shape")) {
        attr(vals$saved_data[[input$data_bank]], "base_shape") <- NULL
      } else if (identical(input$pick_elayers, "layer_shape")) {
    ...
    observeEvent(ignoreInit = TRUE, input$ddcogs1_pick, {
      vals$ddcogs1_pick <- input$ddcogs1_pick
    })

    observeEvent(ignoreInit = TRUE, input$dfcogs1_pick, {
      vals$dfcogs1_pick <- input$dfcogs1_pick
    })

    observeEvent(ignoreInit = TRUE, input$ddcogs3_edit, {
      vals$ddcogs3_edit <- input$ddcogs3_edit
    })

    observeEvent(ignoreInit = TRUE, input$ddcogs3, {
      showModal(removechange())
    })

    observeEvent(ignoreInit = TRUE, input$ddcogs1_rename_remove, {
    ...
    observeEvent(ignoreInit = TRUE, input$ddcogs1, {
      showModal(
        modalDialog(
          size = "m",
          easyClose = TRUE,
          ...
          div(
            ...
            span(
              ...
              shinyBS::bsButton(ns("ddcogs1_rename_go"), "Rename"),
              modalButton("Dismiss")
            )
          )
        )
    ...
    observeEvent(ignoreInit = TRUE, input$dfcogs1, {
      showModal(
        modalDialog(
          size = "m",
          easyClose = TRUE,
          ...
          div(
            ...
            span(
              ...
              shinyBS::bsButton(ns("dfcogs1_rename_go"), "Rename"),
              modalButton("Dismiss")
            )
          )
        )
    ...
    observeEvent(ignoreInit = TRUE, input$ddcogs1_rename_go, {
    ...
    observeEvent(ignoreInit = TRUE, input$dfcogs1_rename_go, {
    ...
    observeEvent(ignoreInit = TRUE, input$comments_remove, {
      attr(vals$saved_data[[input$data_bank]], "notes") <- NULL
    })

    observeEvent(ignoreInit = TRUE, input$comments_edit, {
      showModal(modal_comment())
    })

    observeEvent(ignoreInit = TRUE, input$confirm_note, {
      vals$note <- gsub("\n", "<br/>", input$note_new)
    ...
    observe({
      ...
      withProgress(min = 1, max = length(to_loop), message = "Restoring", {
        for (i in to_loop) {
          ...
          if (isTRUE(restored)) vals$update_state[[i]] <- NULL
        }
      })
    })
  })
