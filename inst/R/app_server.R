...
# Validate input data for mapping modules and toggle UI elements accordingly.
  ...
  output$publication_pdf<-renderUI({
    req(identical(input$intro_tabs,"intro7"))
    tags$iframe(
  ...
  ")),
            div(
              ...
              div(
                ...
                h4(
                  icon("clock"),
                  " Temporal attributes and preprocessing tools"
                ),

                div(
                  strong("Temporal-Attribute"),
                  " is now supported as a dedicated Datalist component for dates, date-times, time-only values, years, months, seasons, and other temporal descriptors."
                ),

                div(
                  "The preprocessing module now includes tools to format/edit temporal attributes, filter rows by temporal attributes, and preserve temporal information when datasets are transformed or aggregated."
                )
              ),
              div(
                ...
                h4(
                  ...
                  " Temporal and spatio-temporal feature engineering"
                ),

                div(
                  "New preprocessing tools can create temporal and spatio-temporal predictors from ",
                  strong("Numeric-Attribute"),
                  ", ",
                  strong("Coords-Attribute"),
                  ", and ",
                  strong("Temporal-Attribute"),
                  " information."
                ),
                ...
                div(
                  "These tools help users derive modelling-ready variables from time, space, and their interaction while keeping the original Datalist structure organized."
                )
              ),
              div(
                ...
                h4(
                  icon("timeline"),
                  " Temporal and spatio-temporal validation"
                ),

                div(
                  "The ",
                  strong("Supervised Algorithms"),
                  " module now supports temporal validation and spatio-temporal validation schemes for model training and evaluation."
                ),

                div(
                  "Users can create temporal block CV, prequential temporal CV, spatiotemporal contiguous block CV, and prequential spatiotemporal block CV schemes, including rolling or expanding temporal windows."
                )
              ),
              div(
                class = "news-card news-card-blue",
...
output$module_sl_server<-renderUI({
  ...
  output$menu_som_out<-renderUI({
    div(imesc_supersom$ui("module_supersom"),
        uiOutput('som_out'))
